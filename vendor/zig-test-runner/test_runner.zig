...
// ── zlsx patch 2 (see README.md) ──
// True only while `runner_test_run` is driving a test under an
// initialised fuzzer. `mainServer` also executes every test once to
// discover which ones are fuzz tests, and during that pass the fuzzer
// runtime does not exist yet — `fuzzer.test_i` is literally `undefined`
// (lib/fuzzer.zig:854), so `fuzzer_new_input`'s `fuzzer.tests[test_i]`
// reads out of bounds and segfaults. Upstream's `fuzz()` cannot tell
// the two passes apart; this flag lets it.
var in_fuzz_run: bool = false;
var fuzz_runner: if (builtin.fuzz) struct {
    ...
    export fn runner_test_run(i: u32) void {
        ...
        // zlsx patch 2: the fuzzer runtime is live for the duration of
        // this call, and only for it.
        in_fuzz_run = true;
        defer in_fuzz_run = false;
...
pub fn fuzz(
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    p: {
                        if (@errorReturnTrace()) |trace| {
                            // ── zlsx patch (see vendor/zig-test-runner/README.md) ──
                            // Upstream 0.16.0 passes `@errorReturnTrace()`
                            // (a `*std.builtin.StackTrace`) straight to
                            // `writeStackTrace`, which takes a
                            // `*const std.debug.StackTrace` — a different
                            // struct. It is a compile error, and it makes
                            // ANY `-ffuzz` build of this runner fail.
                            //
                            // Convert instead. `builtin.StackTrace.index`
                            // counts addresses written and may exceed the
                            // buffer length when the trace wrapped, which
                            // is exactly what `skipped` encodes.
    ...
    // zlsx patch 2: `in_fuzz_run` gates the fuzzer-ABI calls below. On
    // the discovery pass `is_fuzz_test = true` above is the whole job —
    // returning here records the test as fuzzable without touching an
    // uninitialised fuzzer.
    if (builtin.fuzz and in_fuzz_run) {
