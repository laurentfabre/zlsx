...
**Editor** (load-modify-save, v0.2.9): open an existing workbook and round-trip changes through `Editor` — append rows, rewrite individual cells via `setCell`/`setCells`, insert/delete rows, insert/delete columns (with span-shifting across cell refs, `<col>` formatting ranges, merged ranges, `<dimension>`, frozen panes, `<autoFilter>`, modern xdr `<drawing>` + legacy VML `<legacyDrawing>` anchors + paired `<comment ref>`, and structured `<tableParts>` — `<table ref>` / inner `<autoFilter>` + `<filterColumn colId>` / inner `<sortState ref>` / `<tableColumn>` add/drop), add/rename/delete sheets, and `setCell`-driven formula / defined-name / hyperlink-location / DV+CF formula rewriting. ZIP-substitution architecture: untouched entries pass through verbatim, only the patched parts re-deflate, so a 67 KB workbook round-trips in ~5 ms. Remaining row/col-edit refusals are schema-invariant only (header-row delete + zero-dimension collapse on a structured table; `state="split"` pixel panes) — a successful save is byte-safe.
