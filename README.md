<p align="center">
  <img src="assets/logo.svg" alt="zlsx" width="220" height="220">
</p>
...
**Writer** (v0.4.0): pragmatic openpyxl-parity styles — bold/italic, font size/name/color, horizontal alignment, wrap text, 19 OOXML fill patterns, 14 border styles × 5 sides, custom number formats, column widths, row heights, freeze panes, auto-filter, merged cell ranges, external + internal hyperlinks, full data-validation family (list / whole / decimal / date / time / textLength / custom with 8 comparison operators), formulas with cached values. Survived 1M-iter deep fuzz on every surface.

**Editor** (load-modify-save, v0.4.0): open an existing workbook and round-trip changes through `Editor` — append rows, rewrite individual cells via `setCell`/`setCells`, insert/delete rows, insert/delete columns (with span-shifting across cell refs, `<col>` formatting ranges, merged ranges, `<dimension>`, frozen panes, `<autoFilter>`, modern xdr `<drawing>` + legacy VML `<legacyDrawing>` anchors + paired `<comment ref>`, and structured `<tableParts>` — `<table ref>` / inner `<autoFilter>` + `<filterColumn colId>` / inner `<sortState ref>` / `<tableColumn>` add/drop with auto-generated `Column<N>` placeholder name on insert, rename via `setCell` on the header row), add/rename/delete sheets, and `setCell`-driven formula / defined-name / hyperlink-location / DV+CF formula rewriting. ZIP-substitution architecture: untouched entries pass through verbatim, only the patched parts re-deflate, so a 67 KB workbook round-trips in ~5 ms. Remaining row/col-edit refusals fall into two buckets: (a) **schema-invariant** — header-row delete + zero-dimension collapse on a structured table, `state="split"` pixel panes; and (b) **pre-condition guards** — `RowEditRequiresCleanSheet` / `ColEditRequiresCleanSheet` (sheet must have no pending in-memory deltas), Excel's 1,048,576-row / 16,384-col caps. A successful save is byte-safe.
...
**In**
- **Read** workbooks — shared strings (with rich-text runs + XML entities), inline strings, numeric / boolean / error / formula-cached cells, UTF-8 throughout, merged-cell ranges via `Book.mergedRanges(sheet)`, external-URL hyperlinks via `Book.hyperlinks(sheet)` (resolved through sheet `_rels`), all data validations via `Book.dataValidations(sheet)` — dropdowns (values entity-decoded), plus `kind` / `op` / `formula1` / `formula2` on numeric, date, time, text-length, and custom variants, rich-text run formatting (bold / italic / color / size / font) via `Book.richRuns(sst_idx)` for entries that used `<r>` wrappers, per-cell style indices via `Rows.styleIndices()` resolving to number-format codes via `Book.numberFormat(style_idx)` (date detection via `Book.isDateFormat(style_idx)`), per-cell typed style accessors `Book.cellFont` / `cellFill` / `cellBorder` / `cellAlignment` (writer-emitted alignment, wrap_text, diagonal-direction flags, and entity-encoded font names all round-trip cleanly; `xsd:boolean` `"1"` and `"true"` lexical forms both accepted), cell comments via `Book.comments(sheet)` (author + plain-text body, decoded)
- **Write** workbooks — strings (SST-deduped), integers, numbers, booleans, empties, multi-sheet; cell styles with fonts, fills, borders, alignment, wrap, number formats; per-sheet column widths, row heights (validated against Excel's (0, 409.5] cap), freeze panes (legacy clamping form + `freezePanes_checked` typed-error variant), auto-filter (A1-validated), merged cell ranges, external-URL hyperlinks (per-sheet `_rels`), internal hyperlinks (`location="Sheet2!A1"`), list-type data validations (dropdowns), number / decimal / date / time / text-length / custom data validations, workbook + sheet-scoped **defined names** via `Writer.addDefinedName` (rejects A1-shaped, R/C, R1C1-shaped, length-capped, and case-insensitive duplicates per scope), formulas with optional cached value (string-cached results emit `t="str"`, not SST index), rich-text cells (per-run bold / italic / color / size / font) via `SheetWriter.writeRichRow`, cell comments (notes) via `SheetWriter.addComment` (emits the full `xl/comments{N}.xml` + `vmlDrawing{N}.vml` + rels stack so Excel renders the yellow indicator), conditional formatting via `SheetWriter.addConditionalFormatCellIs` / `…Expression` (two core cfRule types) referencing differential formats registered via `Writer.addDxf`. All user-text channels (cells, formulas, comments, URLs, rich-text runs, font names) reject XML 1.0 forbidden control bytes up-front so a stray NUL never produces an unreadable workbook; rows are atomic on error (no half-written `<row>`). OOXML schema-correct on emit: `[Content_Types].xml` Defaults precede Overrides, styles.xml `<cellStyles>` sits between `<cellXfs>` and `<dxfs>`
...
- **Package layer** (`zlsx_pkg`) — OOXML package access independent of the full reader. `PartStore.open(path)` enumerates parts, parses `[Content_Types].xml` + every `_rels/*.rels`, exposes `part(name)` / `rels(owner)` / `resolve(owner, target)` (rejects `TargetMode="External"` and URL-shaped targets). Byte-preserving `save(path)` + `replacePart(name, bytes)` + `addPart(name, content_type, bytes)` for callers that need to round-trip OOXML packages without going through `Editor`. ZIP-bomb / Zip64 / split-archive defenses + eager CRC32 + ZIP32 sentinel-safe save. Drawing-XML helpers `imageAnchors(store)` and `chartAnchors(store)` surface every embedded image/chart with per-sheet anchor coordinates, content-type detection (Strict + Transitional OOXML), and flattened series-ref list. Symmetric XML-entity decoding (named + numeric refs) on every attribute that's compared against ZIP entry names.
- **Unicode-aware sheet-name dedup** — `Writer.addSheet` and `Editor.{addSheet,renameSheet}` use NFC(casefold(name)) so `café` / `CAFÉ` / `cafe + combining-acute`, `ß` / `SS`, `Σ` / `σ` / `ς` all collapse to the same canonical key per Excel's actual semantics. ASCII fast path. Length cap is 31 Unicode scalars, not 31 bytes
- XML entity decoding (`&amp;`, `&lt;`, `&gt;`, `&quot;`, `&apos;`, `&#N;`, `&#xN;`) on read and escaping on write
- CLI (`zlsx {rows|cells|meta|list-sheets|comments|validations|hyperlinks|styles|sst|append-rows|set-cell|insert-row|delete-row|insert-column|delete-column|add-sheet|rename-sheet|delete-sheet} <file>`) emitting uniform NDJSON envelopes, with row/range/header/skip-take/all-sheets/sheet-glob/include-blanks/with-styles/output-mode flags and SIGPIPE-clean pipelines — see "CLI" below and [`docs/jq-for-excel.md`](docs/jq-for-excel.md); C ABI (`libzlsx.{dylib,so,dll}` + `include/zlsx.h`); Python bindings (`pip install py-zlsx`)
...
- No pivot-aware edits — pivot caches and pivot tables (`xl/pivotCache/*`, `xl/pivotTables/*`) carry their own cross-part ref graph that zlsx does not rewrite. Workbooks with pivots round-trip preserved through `Editor` / `PartStore`, but row/col edits won't update pivot ranges. `Pivots` is the last row/col-edit refusal axis still on `docs/plans/refusal-audit.md`; lifting is out of scope while zlsx's writer never emits pivots
- Image extraction (`zlsx_pkg.imageAnchors` / `chartAnchors`, Strict + Transitional OOXML, non-canonical namespace prefixes, both `<xdr:twoCellAnchor>` and `<xdr:absoluteAnchor>`) and minimal image emission (`Workbook.addImage` — oneCellAnchor at zero offset, fixed 1″×1″ extent, drawingless-sheets-only — shipped in v0.3.x via `pkg/drawing_emit.zig`) are in. Typed chart emit (D2) and richer image-anchor controls remain deferred
...
`t` ∈ `"str"` | `"int"` | `"num"` | `"bool"` | `"blank"` | `"date"` | `"error"` | `"formula"`; empty cells skipped from the `cells` array unless `--include-blanks` is set. Date cells (iter61-a) carry an extra `serial` field with the raw Excel serial alongside the ISO `v`. Formula cells (iter61-b) carry `formula` (own text) or `formula_ref` (shared-formula base ref) plus an optional `cached` value instead of `v`. Error cells (iter61-c) carry the literal Excel error string in `v` (`"#DIV/0!"`, `"#N/A"`, etc.). Both 1900 and 1904 (`<workbookPr date1904="1"/>`) date epochs decode correctly — the raw `serial` field always carries the source value unshifted.
...
|---|---|---|---|
| **zlsx** | **3.3 ms** | **2.37 MB** | **1.00×** |
| calamine-rust 0.26 | 3.6 ms | 3.09 MB | 1.11× slower |
| python-calamine 0.6 | 21.0 ms | 17.00 MB | 6.4× slower |
| openpyxl 3.1 (read_only) | 121.8 ms | 42.46 MB | 36.9× slower |

zlsx leads calamine-rust on **every corpus file** (1.02-1.11× faster), holds the **smallest RSS of the four** on every workload (~1.3× below calamine, 7-18× below the Python stack), and beats every Python option 6-37×. See [docs/benchmarks.md](docs/benchmarks.md) for the per-file breakdown.

### Write — 20-run `hyperfine -N` median writing 1,001 styled rows × 10 cols

| Library | Time | Peak RSS | Output | Speedup |
|---|---|---|---|---|
| **zlsx Writer** | **6.7 ms** | **4.44 MB** | 54.9 KB | **1.00×** |
