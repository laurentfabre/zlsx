...
- **Package layer** (`zlsx_pkg`) — read-only OOXML package access independent of the full reader. `PartStore.open(path)` enumerates parts, parses `[Content_Types].xml` + every `_rels/*.rels`, exposes `part(name)` / `rels(owner)` / `resolve(owner, target)`. Byte-preserving `save(path)` + `replacePart(name, bytes)` for callers that need to round-trip OOXML packages without going through `Editor`. Drawing-XML helpers `imageAnchors(store)` and `chartAnchors(store)` surface every embedded image/chart with per-sheet anchor coordinates + chart-type detection + flattened series-ref list.
