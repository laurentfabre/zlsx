...
**Editor** (load-modify-save, v0.2.9): open an existing workbook and round-trip changes through `Editor` — append rows, rewrite individual cells via `setCell`/`setCells`, insert/delete rows, insert/delete columns (with span-shifting across cell refs, `<col>` formatting ranges, merged ranges, `<dimension>`, frozen panes, `<autoFilter>`, modern xdr `<drawing>` + legacy VML `<legacyDrawing>` anchors + paired `<comment ref>`, and structured `<tableParts>` — `<table ref>` / inner `<autoFilter>` + `<filterColumn colId>` / inner `<sortState ref>` / `<tableColumn>` add/drop with auto-generated `Column<N>` placeholder name on insert, rename via `setCell` on the header row), add/rename/delete sheets, and `setCell`-driven formula / defined-name / hyperlink-location / DV+CF formula rewriting. ZIP-substitution architecture: untouched entries pass through verbatim, only the patched parts re-deflate, so a 67 KB workbook round-trips in ~5 ms. Remaining row/col-edit refusals fall into two buckets: (a) **schema-invariant** — header-row delete + zero-dimension collapse on a structured table, `state="split"` pixel panes; and (b) **pre-condition guards** — `RowEditRequiresCleanSheet` / `ColEditRequiresCleanSheet` (sheet must have no pending in-memory deltas), Excel's 1,048,576-row / 16,384-col caps. A successful save is byte-safe.
...
- No formula-aware edits yet — `Editor.insertRow` / `deleteRow` / `insertColumn` / `deleteColumn` refuse globally if any sheet contains a formula or `<definedName>`. The C1 milestone-1 tokenizer (`src/formula/tokenizer.zig`) shipped as the foundation: it gives a flat token stream + loss-preserving printer covering A1 refs (case-insensitive), sheet qualifiers (bare + quoted), ranges, names with function-call disambiguation, every literal/operator class, array constants, and whitespace. The reference-rewriter that uses it to liberate the structural-edit refusals is queued as C1 M2+
