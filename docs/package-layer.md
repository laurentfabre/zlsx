...
| `imageAnchorsIn(store, alloc, mode)`, `chartAnchorsIn(store, alloc, mode)` | The same walks with an explicit `WalkMode`. `.lenient` is the wrappers' historical contract — skip what does not parse, no crash. `.strict` refuses with `error.MalformedDrawingXml` instead: a drawing / image / chart relationship that dangles or is of the wrong relationship type, a named part that is absent, a `<drawing>` element whose reference cannot be read, an anchor whose `from` / `to` / `pos` / `ext` does not parse (a one-cell anchor's schema-required extent included), an opened anchor block or `<c:f>` carrier that never closes. In both modes, plot-type elements and formula carriers inside comments / CDATA / PIs are text, not markup, and the sheet's `<drawing>` element and its id attribute are matched under any namespace prefix that is BOUND to the SpreadsheetML main / office-document relationships namespaces (Transitional or Strict) — a foreign-bound `foo:drawing` / `foo:id` is not the element / not a reference. Strict relationship-type checks accept exactly `{known root}{leaf}` for the Transitional, Strict, and Microsoft-extension roots, so a foreign URI merely ending in `/drawing` is not a drawing edge. The S3b `anchors` NDJSON view reads strict. |
