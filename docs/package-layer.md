...
| Symbol | Notes |
|---|---|
| `imageAnchors(store, alloc)` | Walks every sheet's `<drawing r:id=...>` chain and returns `[]ImageAnchor` with `image_part_name` + `sheet_part_name` + cell-grid `from`/`to` anchors + image bytes. Surfaces `<xdr:absoluteAnchor>` images via the optional `absolute: ?AbsoluteAnchor` field (pixel-coordinate `x` / `y` / `cx` / `cy` in EMUs). Each anchor also carries `kind` (the wrapper as spelled: `two_cell` / `one_cell` / `absolute`) and `doc_offset` (byte offset of the opening tag in the drawing part — the document-order key; the per-prefix replay means slice order alone is not document order). |
| `chartAnchors(store, alloc)` | Same shape for `<xdr:graphicFrame>` containing `<c:chart>`. Each anchor exposes `chart_type` (`bar` / `line` / `pie` / `scatter` / `area` / `bubble` / `radar` / `other`; a compound chart — two or more plot types overlaid — reports `other`) + `series_refs` (every `<c:f>` formula ref flattened in document order, across both Transitional and Strict chart-prefix bindings) + `raw_xml` + `kind` + `doc_offset`. Also carries `absolute: ?AbsoluteAnchor` for pixel-anchored charts. |
| `imageAnchorsIn(store, alloc, mode)`, `chartAnchorsIn(store, alloc, mode)` | The same walks with an explicit `WalkMode`. `.lenient` is the wrappers' historical contract — skip what does not parse, no crash. `.strict` refuses with `error.MalformedDrawingXml` instead: a drawing / image / chart relationship that dangles or is of the wrong relationship type, a named part that is absent, a `<drawing>` element whose reference cannot be read, an anchor whose `from` / `to` / `pos` / `ext` does not parse (a one-cell anchor's schema-required extent included), an opened anchor block or `<c:f>` carrier that never closes. In both modes, anchors, their closes and structural children (`pic`, `graphicFrame`, `from`/`to`, blips), plot-type elements and formula carriers are located as LIVE exact QNames — markup-shaped text inside comments / CDATA / PIs is text (a commented anchor is not inventory, a commented close cannot truncate a live one), a longer element name is not the element, and XML `Eq` whitespace before a carrier tag's `>` is legal — and the sheet's `<drawing>` element and its id attribute are matched under any namespace prefix that is BOUND to the SpreadsheetML main / office-document relationships namespaces (Transitional or Strict) — a foreign-bound `foo:drawing` / `foo:id` is not the element / not a reference, and a binding is only a real attribute in a live opening tag, never declaration-shaped text in a value or comment. The one deliberate exemption: the canonical `r:` prefix is honoured as spelled, the convention every relationship reference in the module reads (`r:embed`, the chart `r:id`). The lookup is one document-order scan across both spellings; a SECOND live drawing element (schema-invalid) refuses under strict, lenient follows the first. Strict relationship-type checks accept byte-exactly `{known root}{leaf}` for the Transitional, Strict, and Microsoft-extension roots, so a foreign URI merely ending in `/drawing` — or a case-mutated leaf — is not a drawing edge. The S3b `anchors` NDJSON view reads strict. |
...
- The DEFAULT namespace is a prefix like any other — the empty one: an unprefixed root in the spreadsheetDrawing namespace (`<wsDr xmlns="…/spreadsheetDrawing"><oneCellAnchor><from><col>`, openpyxl 3.1's spelling) is walked under bare tag names, and a default declaration beside a prefixed root (or on a descendant anchor) is an alternate. A chart part bound the same way (`<chartSpace xmlns="…/chart"><f>`) is walked under `<f>`.
- A spreadsheetDrawing binding under a name the walk cannot spell — longer than 100 bytes, or past the eight-alternate cap — is `DrawingPrefixes.xdr_rejected`: the strict read (`imageAnchorsIn` / `chartAnchorsIn` with `.strict`, the anchors NDJSON) refuses `MalformedDrawingXml` rather than list a partial inventory; the lenient read lists what it can. `pkg/drawing_edit.zig` resolves the same prefixes for the row / column sweep and refuses the edit on the same bytes.
- A wrapper and its children may be spelled under different followed names (`<xdr:twoCellAnchor><from><col>`, both bound to the URI): the wrapper is matched under each tag set, its children under any (`buildTagSets`, `anyLiveExactTag`, `parseCornerIn` — the one corner parser, whose scalar spans the sweep splices through). The empty prefix's scan opener is the bare `<`, so the no-match advance is region-aware (`advancePastNonAnchor`): the comment / CDATA / PI skipping above holds under every spelling.
...
| Symbol | Notes |
|---|---|
| `Workbook.recalculate(alloc, io, run, opts)` | §5.7's in-memory transaction. Recalculates every formula cell and swaps the result in as the final pipeline operation. No file is opened; on any refusal or cancellation the workbook is exactly as it was. Refuses `RecalcRequiresReopen` on `markRecalcOnLoad`'s rule (a mutator installed into the live generation), once the run is known to build a candidate — a workbook with nothing to recalculate is not refused. |
| `Workbook.saveWithRecalc(alloc, io, path, run, opts)` | The same pipeline plus §5.7.9's file transaction: serialize from the *unswapped* candidate → temp write → `File.sync` → final cancellation poll → rename → swap → directory fsync. Any failure before the rename leaves both the destination's prior bytes and the workbook's memory untouched. Refuses `RecalcRequiresReopen` on `markRecalcOnLoad`'s rule when the run would build a candidate (a workbook with nothing to recalculate is the plain save of the live store, plans applied). The file is the plain save plus the recalc (the save-plan fold, 2026-09-11): a staged `setCell` or defined name is rendered into the candidate (`Workbook.foldSavePlansInto`) and drained at the swap — byte-identical to `recalculate` then `save` where no pivot cache is written into (a cache a write lands in takes the refresh marker alone), the plans staged as they were after any failure before the rename (the candidate arm's promise; the arm with nothing to recalculate is a plain save, whose failure leaves the plans applied in memory). What the fold materialized is a save's install: the next transaction on that workbook that would build a candidate refuses `RecalcRequiresReopen` — save and re-open (a transaction after one that carried nothing stays legal, and re-derives the earlier run's patches from the archive as opened — the recorded revert, `c-abi-status-v1.md` §22: one transaction per open, or save and re-open between two). Appended rows stay `SheetHasUnsavedAppends`. |
...
| `Workbook.markRecalcOnLoad()` | Set `<calcPr fullCalcOnLoad="1">` and change nothing else. Honestly named: it does not calculate. Refuses `RecalcRequiresReopen` after a structural edit, an embedding write / prune / strip, an image added, a doc-props strip or a save that materialized cell writes (the recalc-transaction guard: the candidate is built from the archive as opened and cannot carry those parts — run the transaction first, or save and re-open). |
