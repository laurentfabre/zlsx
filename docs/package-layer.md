...
| `Workbook.saveWithRecalc(alloc, io, path, run, opts)` | The same pipeline plus §5.7.9's file transaction: serialize from the *unswapped* candidate → temp write → `File.sync` → final cancellation poll → rename → swap → directory fsync. Any failure before the rename leaves both the destination's prior bytes and the workbook's memory untouched. Refuses `RecalcRequiresReopen` on `markRecalcOnLoad`'s rule when the run would build a candidate (a workbook with nothing to recalculate writes the live store's parts), and `SheetHasUnsavedMutations` over a staged `setCell` on any sheet — neither arm writes one; save first, or `recalculate` then `save`. |
