...
| `imageAnchorsIn(store, alloc, mode)`, `chartAnchorsIn(store, alloc, mode)` | The same walks with an explicit `WalkMode`. `.lenient` is the wrappers' historical contract — skip what does not parse, no crash. `.strict` refuses with `error.MalformedDrawingXml` instead: a drawing / image / chart relationship that dangles or is of the wrong relationship type, a named part that is absent, a `<drawing>` element whose reference cannot be read, an anchor whose `from` / `to` / `pos` / `ext` does not parse (a one-cell anchor's schema-required extent included), an opened anchor block or `<c:f>` carrier that never closes. In both modes, plot-type elements and formula carriers inside comments / CDATA / PIs are text, not markup, and the sheet's `<drawing>` element and its id attribute are matched under any namespace prefix. The S3b `anchors` NDJSON view reads strict. |
