...
| **Frozen panes** | `recordColEdit` per-sheet `<pane ` scan | `ColEditUnsafeForSheet` | `<pane xSplit=…/ySplit=…/topLeftCell=…>` carries column/row coordinates. No rewriter exists; lifting would silently move the freeze line to the wrong boundary. Could be added later (the rewrite is mechanical) but no rewriter is wired today. |
