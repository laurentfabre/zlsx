...
| **`<extLst>` extensions (`x14:`/`x15:`)** | `preflightExtensionEditsForSheet` (`<xm:f>` presence scan) | `ExtensionEditUnsafe` → `RowEditUnsafeForSheet` / `ColEditUnsafeForSheet` | ◐ **Partially lifted 2026-07-29 (#140).** `xm:sqref` element text (the range an extension covers — same meaning in every extension that carries it, appears nowhere else) is shifted by leaf-element name via `shiftRefOrRange`, so `x14:conditionalFormatting` / `x14:dataValidation` / `x14:sparklineGroup` / `x14:ignoredErrors` stay aligned after row/col edits, and a future `x14:` element carrying `xm:sqref` is correct for free. Failure posture: an unterminated element or unparseable ref emits the original bytes — a partially-shifted list would be worse than an unshifted one. `<xm:f>` (sparkline formula ranges) stays **refused**: it needs the formula rewriter plus a sheet-name context `sheet_edit.zig` does not have. Half-closing would be worse than the original bug — shifting `xm:sqref` while leaving `xm:f` stale produces a workbook that looks maintained. Routing `<xm:f>` through the rewriter is the lift that removes this guard. |
...
2026-07-29 (#140)**: `xm:sqref` shifts, `<xm:f>` refuses (see the
