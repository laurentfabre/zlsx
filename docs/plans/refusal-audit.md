...
|---|---|---|---|
| **Drawings (xdr)** | `recordRowEdit` / `recordColEdit` per-sheet `<drawing` scan | `RowEditUnsafeForSheet` / `ColEditUnsafeForSheet` | DrawingML anchors (`<xdr:from>`/`<xdr:to>` with `<xdr:col>`/`<xdr:row>`) carry cell coordinates in a separate part (`xl/drawings/drawingN.xml`). No rewriter wired today (queued as iter dr-1). |
| **Legacy drawings (VML)** | `recordRowEdit` / `recordColEdit` per-sheet `<legacyDrawing` scan | `RowEditUnsafeForSheet` / `ColEditUnsafeForSheet` | VML `<x:Anchor>L,LO,T,TO,R,RO,B,BO</x:Anchor>` (8 ints in `xl/drawings/vmlDrawingN.vml`) carries cell coordinates with pixel offsets. No rewriter wired today (queued as iter dr-2). |
...
| **Pivots / tableParts** | `recordColEdit` per-sheet scan | `ColEditUnsafeForSheet` | Pivot caches and table ranges are byte-encoded across multiple parts (`xl/pivotCache/*`, `xl/tables/*.xml`). No rewriter exists and the cross-part ref graph is non-trivial. Stay refused indefinitely. |
