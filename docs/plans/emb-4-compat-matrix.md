...
| Apple Numbers                     | 14.5 / 15.3 | clean | **stripped**  | **3 STRIPPED**     | Verified 2026-05-30 (14.5), re-confirmed 2026-07-27 on 15.3. File▸Export To▸Excel rebuilds the archive and drops all parts + the rel. Numbers also strips the **ER recovery record** (emb-4B, 5 of 6 carriers) — a Numbers export reports `absent`, not `stripped`. Export must target a non-TCC folder (`~/`, not `~/Documents`). Drive it via `open -a Numbers` + `export document 1`; the `open` Apple event hangs. |
