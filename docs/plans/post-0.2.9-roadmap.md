...
| **Refusal lift: autoFilter** | ✅ shipped 2026-05-14 — `pkg/sheet_edit.zig::processAutoFilterTagRow`/`processAutoFilterTagCol` shifts the row/col halves of `<autoFilter ref>` during the byte transform; full-range collapse drops the element; `<filterColumn colId="N">` children rebase to `new_abs - new_tl_col` on col edits, with the filterColumn at the deleted column dropped entirely. Editor row+col guards no longer scan for `<autoFilter`. 11 pure-function tests + 4 Editor round-trip tests pin the lift. Caveat (table-inner `<sortState>`): closed by the tableParts lift below; sheet-bare open-form autoFilter `<sortState>` still uncovered — third-party files only, zlsx's writer never emits open-form autoFilter. |
| **Refusal lift: structured tables (`<tableParts>`)** | ✅ shipped 2026-05-14 (iter tbl-1) — new `pkg/table_edit.zig::applyEditToTable` walks each `xl/tables/tableN.xml` referenced from the sheet's `<tableParts>` block (resolved through sheet rels). Shifts `<table ref>`, inner `<autoFilter ref>` (delegates to the existing sheet_edit autoFilter rewriter — `<filterColumn colId>` rebase included), and inner `<sortState ref>` — closing the prior autoFilter `<sortState>` caveat at the table-inner scope. On col edits, drops the matching `<tableColumn>` on a delete and adds a synthetic `<tableColumn id="max+1" name="ColumnN"/>` on an insert; `<tableColumns count=>` updates in step. `<table id>` is preserved (workbook-unique field id, not a position). Wired through `Workbook.applyTableEditsForSheet`. Editor pre-flight via `Workbook.preflightTableEditsForSheet` dry-runs the transform per table part BEFORE any sheet bytes are mutated, surfacing the typed errors `TableCollapseUnsafe` (delete that zeros a dimension) and `TableHeaderRowDeleteUnsafe` (delete on the table's top row when `headerRowCount >= 1`, the default) as `RowEditUnsafeForSheet` / `ColEditUnsafeForSheet`. The all-or-nothing contract holds. 12 pure-function tests + 5 Editor round-trip tests (corpus: `tests/corpus/poi_xxe_in_schema.xlsx`, table at C9:E10) pin the lift. New `Workbook.Error` variants: `MalformedTableXml`, `TableCoordinateOverflow`, `TableCollapseUnsafe`, `TableHeaderRowDeleteUnsafe`. v1 limitations: `<extLst>` table extensions pass through verbatim; `totalsRowCount > 0` BR-row deletes are not refused (Excel recomputes on open); `headerRowCount > 1` still refuses any top-row delete regardless. With this iter, **every row/col-edit refusal axis with a rewriter is now lifted** — only the Pivots row in `docs/plans/refusal-audit.md` remains, and that's intentional (zlsx's writer never emits pivots). |
