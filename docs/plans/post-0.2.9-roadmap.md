...
| **B2** Editor rebase onto Workbook | 🚧 in flight — [plan](editor-rebase.md). iter-er-0 ✅ #65 (Editor relocated to `pkg/editor.zig`; nested-types hoist #61); iter-er-1 ✅ #66 (`Editor.open` builds a `Workbook` view via `Workbook.fromBook`); iter-er-2 ✅ #68 (existing-sheet `setCell` routes through `Worksheet.setCell`; `emitWithDeltas` added in #67); iter-er-3 (1/2) ✅ #71 (`Worksheet.appendRows` API surface + symmetric `setCell`/`appendRows` mutual-exclusion guards; baseline harness #69; writer Huffman fix #70 unblocks 100k bench at 174 ms median); iter-er-3 (2/2) ✅ #73 (consumer wiring: `Worksheet.emitWithAppends` substring-splice fast-path + `Worksheet.resolvePartName` parse-free helper + `AppendOutcome` deep-copy ownership; Editor.appendRows shim routes existing sheets through Workbook; 5 Editor.save sites migrated; bench median 176.72 ms = 1.016× of 174 ms baseline, well under the 1.10× / 191.4 ms walk-away ceiling — gate green; two-round zig-defensive audit, all critical findings addressed). iter-er-4 (sheet-level mutators) ✅ #74 + #77 (`Workbook.addSheet` typed surface + `PartStore.replacePart` rels-cache refresh + `PartStore.isOverridden`; Editor.{addSheet, deleteSheet, renameSheet, scanWorksheet} migrated to typed-overlay; legacy `pending_new_sheets` / `pending_renames` queues no longer populated; orphan-aware path-num allocator; `Workbook.addSheet` collision check decodes XML entities). iter-er-4 row/col edits + iter-er-5..7 ⏳ pending. |
