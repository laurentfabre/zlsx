...
| Refusal: row / col edit on a sheet that **hosts** a pivot | ⛔ `PivotEditUnsafe` ¹⁰ | — S3a | — S3a | ⛔ `RowEditUnsafeForSheet`, `ColEditUnsafeForSheet` ¹⁰ | S6 · S7a · S7b · S7c |
| Refusal: row / col edit on a sheet with `<xm:f>` sparkline formulas | ⛔ `ExtensionEditUnsafe` | — S3a | — S3a | ⛔ `RowEditUnsafeForSheet`, `ColEditUnsafeForSheet` ¹⁰ | S2 |
...
¹⁰ `Workbook.preflightPivotEditsForSheet` walks the relationships **of the edited sheet** and refuses when one points at a pivot part — it catches sheets that *host* a pivot. A sheet a pivot only *reads from* (named by `worksheetSource` inside the cache definition) carries no such relationship and is **not detected today**; auditing that is S6's first job. `Editor` remaps both `PivotEditUnsafe` and `ExtensionEditUnsafe` to `RowEditUnsafeForSheet` (row edits) / `ColEditUnsafeForSheet` (column edits); the CLI reports those with exit 3.
