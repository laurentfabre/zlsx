...
| Refusal: row / col edit inside a hosted pivot's footprint, on a host sheet a pivot also **reads from**, or on a pivot graph that cannot be read | ⛔ `PivotEditUnsafe` ¹⁰ | — S3a | — S3a | ⛔ `RowEditUnsafeForSheet`, `ColEditUnsafeForSheet` ¹⁰ | S7b · S7c |
