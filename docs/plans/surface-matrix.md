...
| Row / col edit on a sheet that **hosts** a pivot — `location@ref` moves in step (S7a); row / col edit on a sheet a pivot **reads from** — `worksheetSource@ref` / `rangeSet@ref` move by range semantics, a table-named or defined-name source with its own carrier (S7b); an edit or a cell write that changes a source's **content** marks its cache `refreshOnLoad="1"` — a proven shift never does (S7b-3); a row edit that changes a source's content **rebuilds the cache** from the cells — records, inventories, `recordCount`, `refreshedDate` — in the same install (S7b-4, `pivots.engine.rebuild`) | ✓ `Workbook.preflightPivotEditsForSheet` ¹⁰ | — S3a | — S3a | ✓ `insert-row` ⁹ | S3a |
| Refusal: row / col edit inside a hosted pivot's footprint; a delete of a source's header row or only row; a column edit inside a source range (S7c); a defined-name source the name sweep would break; a source spelling that claims the sheet and cannot be moved; a content change the engine cannot rebuild from — no finite rectangle, a calculated / group / OLAP shape, a cell the oracle matrix has not covered (S7b-4); a pivot part two sheets host; a pivot graph that cannot be read | ⛔ `PivotEditUnsafe` ¹⁰ | — S3a | — S3a | ⛔ `RowEditUnsafeForSheet`, `ColEditUnsafeForSheet` ¹⁰ | S7b · S7c |
...
| S7b | **Reference move, marker and cache rebuild landed 2026-08-29** (S7b-1 typed bounds, S7b-2 the `ref` move, S7b-3 `refreshOnLoad`, S7b-4 the engine's first slice: the cache rebuilt from the cells, unsupported shapes refused) — §3 the source-sheet row (footnote ¹⁰); the consumers' layout and output cells (S7b-5) follow. C and Py observe it through S3a. |
| S7c | §3 the remaining `PivotEditUnsafe` lift — cache fields under column edits — staged. |
