...
| Q3 | **Yes — one rule, S7b's second PR.** | Cell writes inside a source rectangle (or on a referenced sheet of an unbounded source) mark at save time; under B they are also candidates for the same rebuild — the second PR decided: a `setCell` triggers the engine where it can lay the form out, the marker alone where it cannot (S7b-5). |
