...
surfaces them): `pkg/drawing_edit.zig` hard-codes `xdr:` prefix (✅ closed 2026-09-05 — the namespace-aware drawing slice);
...
| **Refusal lift: structured tables (`<tableParts>`)** | ✅ shipped 2026-05-14 (iter tbl-1) — new `pkg/table_edit.zig::applyEditToTable` walks each `xl/tables/tableN.xml` referenced from the sheet's `<tableParts>` block (resolved through sheet rels). Shifts `<table ref>`, inner `<autoFilter ref>` (delegates to the existing sheet_edit autoFilter rewriter — `<filterColumn colId>` rebase included), and inner `<sortState ref>` — closing the prior autoFilter `<sortState>` caveat at the table-inner scope. On col edits, drops the matching `<tableColumn>` on a delete and adds a synthetic `<tableColumn id="max+1" name="Column<id>"/>` on an insert (collision-avoiding: probes `Column<id>_2`, `_3`, ... if the bare name is taken — REL-A502); `<tableColumns count=>` updates in step (refuses with `MalformedTableXml` on count/range divergence — REL-A505). `<table id>` is preserved (workbook-unique table id, not a position; distinct namespace from `<tableColumn id>` which is table-unique). `headerRowCount="0"` (explicit headerless table) permits top-row delete; absent or non-zero refuses (REL-A501). Wired through `Workbook.applyTableEditsForSheet`. Editor pre-flight via `Workbook.preflightTableEditsForSheet` dry-runs the transform per table part BEFORE any sheet bytes are mutated, remapping every table_edit typed error (`TableCollapseUnsafe`, `TableHeaderRowDeleteUnsafe`, `MalformedTableXml`, `TableCoordinateOverflow`) to `RowEditUnsafeForSheet` / `ColEditUnsafeForSheet` so Editor's public surface stays consistent (REL-B527/B528). `TablePartRidIterator` bounds `tag_end < block_end` against adversarial input (REL-B524). The all-or-nothing contract holds. 20 pure-function tests + 5 Editor round-trip tests (corpus: `tests/corpus/poi_xxe_in_schema.xlsx`, table at C9:E10) pin the lift; the byte-stable test asserts `expectEqualSlices` on the unaffected table part (REL-B530). New `Workbook.Error` variants: `MalformedTableXml`, `TableCoordinateOverflow`, `TableCollapseUnsafe`, `TableHeaderRowDeleteUnsafe`. v1 limitations: `<extLst>` table extensions pass through verbatim; `totalsRowCount > 0` BR-row deletes are not refused (Excel recomputes on open); `headerRowCount > 1` (spec-legal but vanishingly rare) still refuses any top-row delete regardless. With this iter, **every row/col-edit refusal axis with a rewriter is now lifted** — only the Pivots row in `docs/plans/refusal-audit.md` remains, and that's intentional (zlsx's writer never emits pivots). |
| **Refusal lift: `<picture>`** | ✅ shipped 2026-05-14 — CT_SheetBackgroundPicture (ECMA-376 §18.3.1.67) is a single coordinate-free `r:id` reference to a tiled background image; row/col edits cannot misalign it. Editor row+col guards drop the `<picture` scan; the element passes through the byte transform unchanged. 2 Editor round-trip tests inject `<picture/>` via `PartStore.replacePart` and confirm insertRow + insertColumn no longer error. dr-1 (modern `<drawing>` rewriter) and dr-2 (VML `<legacyDrawing>`) are the remaining drawings-axis lifts. |
| **Refusal lift: `<drawing>` (xdr)** | ✅ shipped 2026-05-14 (dr-1) — new `pkg/drawing_edit.zig::applyEditToDrawing` walks `<xdr:twoCellAnchor>` and `<xdr:oneCellAnchor>` blocks inside `xl/drawings/drawingN.xml`, shifting `<xdr:col>` / `<xdr:row>` 0-based coords on the matching axis. Full-collapse anchors drop entirely. `<xdr:absoluteAnchor>` (pixel coords) passes through. Wired through `Workbook.applyDrawingEditForSheet` in `applySheetEditTransform` — drawing-part rewrite runs alongside the worksheet byte transform; no-op rewrites are byte-identical so SHA256 passthrough holds. New `Workbook.Error` variants: `MalformedDrawingXml`, `DrawingCoordinateOverflow`. 12 pure-function tests on the byte transform + 3 Editor round-trip tests via `Workbook.addImage`. v1 limitation: hard-codes `xdr:` prefix; namespace-aware support (cf. `pkg/drawings.zig`) queued (✅ closed 2026-09-05 — the namespace-aware drawing slice). `<legacyDrawing>` (VML, dr-2) remains refused upstream. |
| **Refusal lift: `<legacyDrawing>` (VML)** | ✅ shipped 2026-05-14 (dr-2) — new `pkg/vml_edit.zig` walks `<v:shape>` blocks in `xl/drawings/vmlDrawingN.vml`. For shapes with `<x:ClientData>`, shifts `<x:Row>` / `<x:Column>` (0-based) AND the matching from/to pair within the 8-int `<x:Anchor>` payload. Drop semantics: when the anchor cell coordinate equals the deleted index, the entire `<v:shape>` block is removed. PAIRED comments part rewrite (REL-705): every VML note has a `<comment ref>` in `xl/commentsN.xml` that MUST stay synchronized — `applyEditToCommentsXml` shifts/drops in step. Wired through `Workbook.applyVmlDrawingEditForSheet`. New `Workbook.Error` variants: `MalformedVmlDrawing`, `VmlCoordinateOverflow`, `MalformedCommentsXml`. 21 pure-function tests + 5 Editor round-trip tests via `Writer.addComment`. Codex-reviewed (3 rounds, 6 real findings + 1 rebase artifact, all fixed). v1 limitation: hard-codes `v:` and `x:` prefixes. **Drawings axis fully lifted.** |
