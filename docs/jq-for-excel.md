...
- `--sst-lazy` — open the workbook with `Book.openSstLazy` so the shared-string table is resolved on demand (mitigates RAM blow-up on workbooks with millions of unique strings). Sparse-access workloads win; full-sweep `zlsx sst` materialises every entry, so it's slightly slower than eager. The deferred decode removes no validation — neither backend refuses a torn `<t>` — but the two read such a table differently (on eager the torn entry swallows markup and the entries after it up to the next `</t>`, every later index shifts; lazy bounds each entry by its `</si>`, keeps its ordinal and yields the text before the tear); what the deferral moves from open to the first access of a plain entry is the entity decode's verdict (a malformed entity is `MalformedXml` at open on the eager openers; a rich-run entry's runs decode at open on both) and the allocation. Mutually exclusive with `--lazy` (exit 1). See `docs/plans/archive/streaming-sst.md`.
- `--lazy` — open the workbook with `Book.openLazy` so only the sheets the sub-command visits are extracted (the inventory, shared strings, styles and theme still load at open): `rows` / `cells` under `--sheet` / `--name` / `--sheet-glob` on a workbook whose other sheets are large is the win; the per-object family (`comments` / `validations` / `hyperlinks` / `merges`) gains under a selector and extracts every sheet without one; `meta` visits every sheet (its `has_comments` is the OR across all of them) and gains nothing; `list-sheets` / `styles` / `sst` extract none. The visited sheets are extracted **before the first record**, so the run's bytes and exit code are the eager opener's on the same selection — a visited sheet the eager opener would refuse at open (its part missing, a side index malformed) is refused at exit 2 with the sheet named and nothing written; a sheet the selection never reaches is never read, so a tear there goes unreported by that run. Accepted on every sub-command of this grammar (`eval` / `recalc` / `dbx` have their own and refuse it), a no-op where no reader `Book` is opened. Mutually exclusive with `--sst-lazy` (exit 1): the reader has no opener with both strategies.
...
1. **Formula cached values**: `t:"formula"` always has `formula`; `cached` field is present only when Excel stored a cached result. Should we auto-recalculate? **Proposal: no** — the read stream never computes. *(Historical: written when zlsx had no engine. Since M6 the answer is still no for `rows`/`cells` — but callers that need computed values now use `zlsx eval` / `zlsx recalc` instead of shelling out to libreoffice / excel. See `cli.md` § Formula.)*
