...
- `--sst-lazy` — open the workbook with `Book.openSstLazy` so the shared-string table is resolved on demand (mitigates RAM blow-up on workbooks with millions of unique strings). Sparse-access workloads win; full-sweep `zlsx sst` materialises every entry, so it's slightly slower than eager. **Caveat**: open no longer fast-fails on per-entry malformed `<t>` content — the error surfaces at first access of that entry instead. Callers that need open-time validation should drop the flag. See `docs/plans/archive/streaming-sst.md`.
