...
> **Status**: design **shipped** — every rollout slice iter54 → iter60c plus iter61-a/b/c is landed on `main` and Codex-reviewed clean. The sub-command dispatch, envelope, pagination/range/header, output modes, signal handling, inline error-record mechanism, AND cell-type completeness (`t:"date"` / `"error"` / `"formula"` with `formula_ref` + `cached` per shared-formula semantics) are all live. Four rounds of design-level Codex review (v1 → v4.1) preceded implementation; each code slice went through its own review rounds. The `t` tag now spans all 8 design-doc values: `str` / `int` / `num` / `bool` / `blank` / `date` / `error` / `formula`. See `src/cli.zig` for the implementation and the README's "CLI" section for the user-facing reference. **Known limitations**: 1904-epoch workbooks (`<workbookPr date1904="1"/>`) skip the `t:"date"` auto-conversion (numeric value preserved) until proper 1904 decoding ships. Array formulas surface only on the top-left cell.
