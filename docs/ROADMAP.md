...
| E6 | Embeddings from the command line | done | 2-3 wk | E5 | Add, prune and strip vectors without writing code. `zlsx embed` ships five mutually exclusive modes: `--extract` (rows needing embedding, as NDJSON), `--vectors PATH` (write them back; `--recovery in-cells` keeps the recovery record through an Apple Numbers export, 2026-09-07), `--prune` (tombstone stale slots), `--strip` (remove parts *and* the recovery record, so the result reports `absent`), `--dump` (the state — present / stripped / absent with its provenance — the coverages and every stored vector with its hash, as NDJSON; the read row's CLI leg, 2026-09-09). |
