...
| **Formulas + cached values** | BLS Employment Situation | [bls.gov/web/empsit.supp.toc.htm](https://www.bls.gov/web/empsit.supp.toc.htm) publishes xlsx with formulas for monthly aggregates. Cached `<v>` values are what the read path wants — it never evaluates. *(Since M6, evaluation exists as its own surface — `zlsx eval` / `recalc` — with its own fixture + oracle suite, `goal_formula.md` §8.)* |
...
| **Cells with `<f>` (formula) children** | The *reader* skips the formula and reads `<v>` (cached value) — still true, by design. *(The formula engine's own `<f>` handling — parse, graph, recalc — is covered by the M0–M6 suites in `src/formula/` + `pkg/`, not this corpus.)* Reader-side: tested in theory via the "ph-poi test1" file but not a dedicated fixture. |
