...
allocation failure (`--vectors`' whole-set buffer before the open, `--dump`'s
one-row buffer before its first record — stdout empty either way). Exit 4 for a breached
