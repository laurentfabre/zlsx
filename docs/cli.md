...
| `embed --dump` | *(none)* | Read-only: the embedding read — what `zlsx.embeddings` and the `zlsx_emb_*` handle report — as NDJSON, three record kinds in this order. First one `{"kind":"embeddings","state":"present"\|"stripped"\|"absent",…}`: `present` and `stripped` carry `model`, `dim`, `dtype` (as the index spells it — `f32`, `int8-sym-per-vec`; a `--vectors --dtype int8-sym` write reads back as the latter), `hash_algo` (`xxh3-64`) and `coverage_count`; `stripped` adds `digest` (the content fingerprint at embed time, an unsigned 64-bit decimal) and `carrier` (`defined_name` / `doc_props` / `cell_data` — which carrier the record survived in); `absent` carries the state alone. Then one `{"kind":"coverage","id":…,"sheet":…,"range":…,"rows":N}` per coverage, in index order — `sheet` the worksheet part as the index names it (`worksheets/sheet1.xml`, not a sheet index), `rows` the slot count; a present coverage adds `column` and `include_formulas` (the record does not carry them). Then, present only, one `{"kind":"vector","coverage":ID,"row":N,"hash":H,"vector":[…]}` per covered row, coverage by coverage in slot order — `row` the 1-based sheet row, `hash` the stored xxh3-64 as an unsigned 64-bit decimal (the `embed_row` caveat on JSON readers that narrow integers), `vector` the row decoded to f32 whatever the dtype (the decoder `zlsx_emb_vectors` uses; each component the single's shortest round-trip digits, `0.1` not `0.10000000149011612`). A tombstoned slot prints `hash` and `vector` as `null` — no vector, by the tombstone contract; the slot keeps its place so the range stays dense and `row` stays aligned. A component that is not a finite number (no zlsx write produces one; a foreign writer can) prints as `null`. A key a state cannot answer is omitted, not `null`. The vector records filtered to `{row, vector}` are a `--vectors` input again — `jq -c 'select(.kind=="vector" and .hash != null) \| {row, vector}'` — and written back with the same `--model`, `--column` and `--coverage` the dump is byte-identical. Nothing reaches stdout before the whole set is read and judged: refuses (exit 3, stdout empty) a set the read cannot serve — `MissingEmbeddingPart` (the index's companion rels file, or a part the index names, gone), the index read's own names (the ones `--prune` folds under `MalformedEmbeddingSet`; here each keeps its name — `CountMismatch`, `DimensionMismatch`, `DtypeMismatch`, `BadMagic`, `UnsupportedVersion`, `BodySizeMismatch`, `InvalidRange`, `InvalidDtype`, …), on a stripped set the record reader's, and `InvalidUtf8` on an index or record string (the C surface copies them out raw; py-zlsx raises on decode). Takes no `--out`. `--recovery` beside it is refused (exit 2) as beside `--extract`; the write's flags (`--model`, `--id`, `--dtype`, `--column`, `--coverage`, `--sheet`) are ignored as beside `--strip` / `--prune` (the recorded owner call). Recorded, every read surface: `model` / `id` / `sheet` / `range` are handed over as the index spells them (an `&` in a model name reads back `&amp;`) |
...
allocation failure (`--vectors`' whole-set buffer, `--dump`'s one-row buffer). Exit 4 for a breached
