...
zlsx embed out.xlsx --dump        # the state, the coverages, every vector with its hash
```

| Mode | Required flags | What it does |
|---|---|---|
| `embed --extract` | `--column A --coverage A2:A100` | Read-only: one `{"kind":"embed_row","row":N,"text":"…","hash":H}` record per embeddable row — `text` as a reader sees the cell (a shared or inline string's runs joined, entities resolved; a number's `<v>` as written; an error's literal, `#N/A`; a boolean as `1` / `0`), `hash` the canonical xxh3-64 content hash `--vectors` stores beside the vector, as an unsigned 64-bit decimal (a JSON reader that narrows integers to doubles — `jq` before 1.7 — rounds it; Python's `json` does not). Rows with nothing embeddable are omitted. Refuses (exit 3) rather than hand over a record that lies: `MissingRelationship` / `MissingSheetPart` (the sheet's part is unreachable), `MalformedSheetXml` (a sheet part the view cannot parse, or a row or cell it cannot place — no `r`, or one it cannot read: 0, non-numeric, past the limit — or a ref under another row; the corpus's positional `wdi_excel.xlsx` refuses here, where it used to read as empty), `MalformedSharedStringsXml`, `UnsupportedCellValue` (a boolean `<v>` that is not 0 / 1, a `<v>` the number canonicalizer cannot read, a `t="d"` ISO-8601 date, a `t` this reader does not know, a shared-string index that is not a number, an entity the decoder does not know), `SstIndexOutOfRange`, `InvalidUtf8`, `UnicodeNormalizationFailed`. A sheet the editor holds staged cell writes for is not this read's concern (the CLI opens fresh) |
| `embed --dump` | *(none)* | Read-only: the embedding read — what `zlsx.embeddings` and the `zlsx_emb_*` handle report, plus three keys only the CLI carries (`hash_algo`, `column`, `include_formulas`) — as NDJSON, three record kinds in this order. First one `{"kind":"embeddings","state":"present"\|"stripped"\|"absent",…}`: `present` and `stripped` carry `model`, `dim`, `dtype` (as the index spells it — `f32`, `int8-sym-per-vec`; a `--vectors --dtype int8-sym` write reads back as the latter), `hash_algo` (`xxh3-64`) and `coverage_count`; `stripped` adds `digest` (the content fingerprint at embed time, an unsigned 64-bit decimal) and `carrier` (`defined_name` / `doc_props` / `cell_data` — which carrier the record survived in); `absent` carries the state alone. Then one `{"kind":"coverage","id":…,"sheet":…,"range":…,"column":…,"include_formulas":…,"rows":N}` per coverage, in index order — `sheet` the worksheet part as the index names it (`worksheets/sheet1.xml`, not a sheet index), `rows` the slot count; a stripped coverage omits `column` and `include_formulas` (the record does not carry them). Then, present only, one `{"kind":"vector","coverage":ID,"row":N,"hash":H,"vector":[…]}` per covered row, coverage by coverage in slot order — `row` the 1-based sheet row, `hash` the stored xxh3-64 as an unsigned 64-bit decimal (the `embed_row` caveat on JSON readers that narrow integers), `vector` the row decoded to f32 whatever the dtype (the decoder `zlsx_emb_vectors` uses; each component the single's shortest round-trip digits, `0.1` not `0.10000000149011612`). A tombstoned slot prints `hash` and `vector` as `null` — no vector, by the tombstone contract; the slot keeps its place so the range stays dense and `row` stays aligned. A component that is not a finite number (no zlsx write produces one; a foreign writer can) prints as `null`. A key a state cannot answer is omitted, not `null`. The vector records filtered to `{row, vector}` are a `--vectors` input again — `jq -c 'select(.kind=="vector" and .hash != null) \| {row, vector}'` — and written back with the same `--model`, `--column`, `--coverage` (and `--id` / `--sheet` where they were given) an f32 set's dump is byte-identical (a record with a `null` component is not such an input — drop it or re-embed the row; an int8 set reads back decoded to f32 and is rewritten as f32 unless `--dtype int8-sym` is given again, and re-quantizing is not bit-exact). Nothing reaches stdout before the whole set is read and judged — every refusal leaves stdout empty, exit 2, 3 and the one-row buffer's 4 alike (pinned under allocation-failure injection on a present and a stripped set; only an output failure mid-stream, exit 5, can leave a prefix behind): refuses (exit 3) a set the read cannot serve — `MissingEmbeddingPart` (the index's companion rels file, or a part the index names, gone), the index read's own names (the ones `--prune` folds under `MalformedEmbeddingSet`; here each keeps its name — `CountMismatch`, `DimensionMismatch`, `DtypeMismatch`, `BadMagic`, `UnsupportedVersion`, `BodySizeMismatch`, `InvalidRange`, `InvalidDtype`, …) and `InvalidUtf8` on an index or record string (the C surface copies them out raw; py-zlsx raises on decode). A recovery record the reader cannot decode is not a refusal: it reads `absent` on every surface (the library's rule — provenance, not data; a malformed record must not make a readable workbook refuse — a record claiming more coverages than any carrier could hold refused `BufferTooSmall` before this slice, and folds now). An allocation failure inside the read is exit 3, as on `--vectors`. Takes no `--out` (one given is ignored, as beside `--extract`). `--recovery` beside it is refused (exit 2) as beside `--extract`; the write's flags (`--model`, `--id`, `--dtype`, `--column`, `--coverage`, `--sheet`) are ignored as beside `--strip` / `--prune` (the recorded owner call). Recorded, every read surface: `model` / `id` / `sheet` / `range` are handed over as the index spells them while present (an `&` in a model name reads back `&amp;`) and as the record decoded them when stripped (the same model reads `&`) — one rule for the two carriers is an owner follow-up on the shared read |
| `embed --vectors P` | `--column A --coverage A2:A100 --model NAME --out P` | Write the embedding parts from `{"row":N,"vector":[…]}` NDJSON; covered rows with no vector become tombstones. Optional: `--id NAME`, `--dtype f32\|int8-sym`, `--sheet N`, `--recovery invisible\|in-cells` (default `invisible`) — where the recovery record rides: `invisible` is the two carriers no user sees (the hidden defined names and `docProps/custom.xml`), which an Apple Numbers export erases (that export then reads `absent`); `in-cells` also writes it into a hidden sheet named `zlsxRecovery` (Excel's *Hide*), the one carrier Numbers keeps, so that export reads `stripped` with its provenance intact — at the cost of a sheet the user can reveal (the two cannot be had together). Governs the sheet's creation only: a workbook already carrying it (by its reserved name, however cased) has its cell refreshed by every write, whatever `--recovery` says; `--strip` removes it. Judged as `--dtype` is — before the input is opened — so an unknown spelling is exit 2 with no output written; with `--extract` / `--prune` / `--strip` the flag is refused (exit 2) rather than ignored. `--sheet`, `--coverage`, `--column`, `--id` and `--model` are the write's own verdicts, judged after the open (exit 3; the input is untouched and no output is written). Refuses (exit 3) before the first part write: `--extract`'s reading verdicts on the covered sheet first (the same names, as `zlsx: embed --vectors: <name>` — the corpus's positional `wdi_excel.xlsx` refuses here), then the write's own, the vocabulary of `zlsx_editor_set_embeddings` (`docs/plans/c-abi-status-v1.md` §18 and §21) — the coverage's rules (`InvalidRange` — the range, or a column outside it — `InvalidCoverageId`, `InvalidXmlByte` in the model name, `EmbeddingExceedsArchiveLimit`; the multi-coverage names `DuplicateCoverageId` / `CoverageOverlap` are the C and Python surfaces' — the CLI writes one coverage), the archive's (`MissingRelationship`, `MissingWorkbookRels` / `MalformedWorkbookRels`, `MalformedWorkbookXml`, `MissingContentTypes` / `MalformedContentTypes`, `IdSpaceExhausted`), the scrub's on every write (`MalformedSharedStringsXml`; `MalformedSheetXml` when the `zlsxRecovery` sheet or an orphaned worksheet part is in the archive) and, when the hidden sheet is created, its creation's (`MissingWorkbookPart` / `SheetCountMismatch`). An allocation failure inside the write is exit 3 too — only the CLI's own vector-buffer allocation is 4. The same option is `flags` bit 0 of `zlsx_editor_set_embeddings` / `Editor.set_embeddings(…, recovery="in_cells")`. Recorded, every surface: a `--sheet` that names the `zlsxRecovery` sheet itself is accepted — the record's cell becomes covered content and its hash is over the record the same write replaces; the refusal belongs to the shared write |
| `embed --prune` | `--out P` | The redaction sweep: tombstone every slot whose row is no longer embeddable and zero its vector (a row deleted in plain Excel leaves its vector on disk — this removes it). Changed-but-present content is reported stale, not redacted; a workbook with no set, or a stripped one, is an all-zero summary. Judges through `--extract`'s reading and refuses (exit 3) the same way it does — a covered cell the read cannot carry, or a row or cell the view cannot place, stops the sweep rather than redact a row that has text — and refuses a set the index read cannot read as `MalformedEmbeddingSet` (a coverage range it cannot parse, a binary part with the wrong magic, a count that disagrees with its header) or `MissingEmbeddingPart` (a part the index names, gone), each before the first part write. The same sweep is `zlsx_editor_prune_embeddings` / `Editor.prune_embeddings()` |
| `embed --strip` | `--out P` | Remove the embedding parts *and* the recovery record from every carrier — the hidden defined names, the `docProps/custom.xml` property, and the `recovery_in_cells` sheet (by its reserved name `zlsxRecovery`, however cased) with its part and the record's string in the shared-string table (blanked in place) — the pre-share operation; the result reports `absent`, not `stripped`. Refuses (exit 3) before the first removal: `MalformedWorkbookXml` (an `xl/workbook.xml` the strip cannot walk) and, when the cells sheet is present, the sheet delete's own verdicts. The same strip is `zlsx_editor_strip_embeddings` / `Editor.strip_embeddings()` |
...
returns 2 for a flag value it judges before the open (`--dtype`, `--recovery`,
a missing required flag, `--recovery` beside a mode other than `--vectors`),
3 on a set `--dump` / `--prune` cannot serve or a row `--extract` cannot
carry (each mode's row above names its verdicts), and 4 on a vector-buffer
allocation failure (`--vectors`' whole-set buffer after the open and the
reading verdicts, before the first part write; `--dump`'s one-row buffer
before its first record — the input untouched and stdout empty either way). Exit 4 for a breached
