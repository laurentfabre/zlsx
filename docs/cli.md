...
|---|---|---|
| `embed --extract` | `--column A --coverage A2:A100` | Read-only: one `{"kind":"embed_row","row":N,"text":"…","hash":H}` record per embeddable row — `text` as a reader sees the cell (a shared or inline string's runs joined, entities resolved; a number's `<v>` as written; a boolean as `1` / `0`), `hash` the canonical xxh3-64 content hash `--vectors` stores beside the vector, as an unsigned 64-bit decimal (a JSON reader that narrows integers to doubles — `jq` before 1.7 — rounds it; Python's `json` does not). Rows with nothing embeddable are omitted. A sheet the editor holds staged cell writes for is not this read's concern (the CLI opens fresh) |
