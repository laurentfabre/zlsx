...
Tier **D1**. One PR per row — **47 rows, M-1 … M10f** (count = the table; every v1 function name frozen — no ellipses);
counts regenerate from the frozen registry inventory (M3a). v1 = M9d; M10a–M10f are the post-v1 rows.
