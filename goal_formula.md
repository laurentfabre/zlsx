...
> measured performance. Companion to `docs/ROADMAP.md`; detail specs in
...
>
> **This file stays at the repo root and is normative for the D1 engine** —
> 33 source files cite its section numbers directly. The §15 audit tables
> below cite `goal.md` / `goal_plan.md` with line anchors; those two files
> were retired into `docs/ROADMAP.md` on 2026-08-14, so those anchors
> address the files as they stood then (recoverable from git history).
...
already a recognized distinct mutation, `docs/plans/archive/structural-edits.md:100`;
...
Tier **D1**. One PR per row — **59 rows, M-1 … M10s** (count = the table; every v1 function name frozen — no ellipses);
counts regenerate from the frozen registry inventory (M3a). v1 = M9d; M10a–M10r are the post-v1 rows. M10q carries no row: it is
instrumentation (the fill/capacity probe), recorded in goal_codex.md §3 rather than here.
...
| **M10i** ✅ | **The record-lists row — the copy instant that was never the peak, and the projection instant that was** (`feat/m10i-scan-records`). The 90.1 MiB re-profiles at the M10h baseline first and reproduces it exactly on all three probes (RSS 96 337 920 B — the recorded bytes; peak live 101 145 606 at 96.5 MiB; the shortlisted site byte-for-byte: 25 514 508 at peak, 59 allocs, 76 434 388 churned, the cells' 13 601 360 exact dupe beside it). A Debug-granularity profile de-inlines the shortlisted site into its three record lists — `cells` 14 511 880 / 22 allocs / 43 487 632 churned, `slots` 10 670 500 / 23 / 31 985 700, `rows` 332 128 / 14 / 961 056 — M10h's predicted attribution, confirmed before cutting. **The row's finding: the copy instant was never the peak.** The shortlist's own cut proved it — handing each list off in place (`toOwnedSlice` remaps to length; the lists are page-backed far over `SmpAllocator`'s slab line, so the shrink munmaps the doubling tail and the exact dupes are never allocated; the dupe-and-free dated from the lists' arena era (M10c), and an arena cannot shrink) removed the copy instant entirely, and `peak_live_bytes` did not move by one byte: the true peak was **the projection instant**, tied at the same 101 145 606 inside the profiler's 1 MiB snapshot-step blind spot, holding 16.9 MB nothing would read again — the bridge's input cells (6 400 000) and the roots (3 840 000), dead since the drive returned but `defer`-held to `prepare`'s end, and the driver's published list (4 480 000) + index (2 228 248), last read by the final published sheet's publication build. Both pairs die at their last readers now (M10f's fold-before-staging pattern, two more seams). The counted pre-walk was NOT taken — the measured answer: with the dupes gone the whole scan era sits below the peak, so exactness there buys zero RSS, and the ladders' 76.4 MB of churn already returns to the OS (the probe reads 7.3 MiB *below* the live site-sum). **Result (§9.1): first-recalc RSS 90.1 → 80.0 MiB — 5.3× the 15.15 MiB ceiling, from 5.9×**; profile site-sum at the peak instant 96.5 → 88.7 MiB, and **the peak instant leaves the patcher for the graph build**: the link block (13 450 007), the CSR edge pair (12 205 998), the owner map (7 680 000), the capture arena (6 551 868 across 17 chunks), keys + captures (5 599 952), over the pre-drive base (model 20 065 940, part bytes 7 951 114, the bridge's input records 6 400 000, shared texts 2 780 178 — the computed layer does not exist yet at this instant). The next memory row is the graph-build era — M10g's exact blocks are now what sets the mark — or the ceiling conversation | Saved archives byte-identical to main across all four workloads; **`compare_bench --gate` exit 0, every lane ok** — F1 named eval +5.3 % lane median (415.6 ms as recalc − open; the 500 ms ceiling stays met; an interleaved head-to-head on the same binaries read 402.9 ± 3.0 vs 403.9 ± 5.0 ms — parity, the lane delta was sequential-run drift), named `save` −2.2 %, criteria eval −7.1 %, text eval −8.8 %, registry eval −9.7 % (largest z 4.0, criteria-eval tiny at +3.2 %, under the gate's compound threshold); zig build + zig build test green (132 steps, 9 842); zig fmt --check clean; the RSS probe byte-reproducible across three runs (85 721 088 B each) |
...
| **M10n** ✅ | **The staging-plateau row — the 264 KB gap that was not the price** (`feat/m10n-cell-spans`). The 69.5 MiB re-profiles at the M10m baseline first and reproduces it exactly (RSS 74 711 040 B ×3; peak live 78 087 308; every era byte-for-byte). **The pre-flight read answered the row's only real question, which was whether to spend it at all.** M10m handed this row a runner-up gap of 264 KB — staging's plateau at 78 087 308 with the projection instant 263 894 B behind it — and a 264 KB row is noise. The trace says that reading is wrong, and says why: the projection era is *the same phase*, holding the same records, so a cut at the plateau lands on both eras at once and neither can become the peak by surviving. **The era that sets the price is the first one the cut does not touch — the drive's publish at 76 794 991.** Budget therefore **1 292 317 B, not 263 894**, and the distinction is structural rather than lucky: an era is only a runner-up if the cut misses it. (The published pair, 3 200 000 + 1 703 960, is the shape that would have been trapped by the 264 KB reading — it lives at the plateau *and* in the drive era, so cutting it pays exactly the gap and stops. It was left alone.) One cut, sized to the budget (output bytes unchanged): **`CellSpans` stops paying the optional's tag.** A `Span` is two `u32`s, so `?Span` costs twelve bytes for eight of content — and `CellSpans` carries six of them (`type_attr`, `v`, `v_content`, `f`, `f_ref`, `is`) while the splice scan keeps one `CellSpans` per `<c>` element, 100 010 of them on the named workload. `OptSpan` states absence as `end < start` — a shape no present span can take, since every span the scanner records runs from an opening byte to a byte at or after it, which `Span.slice` already asserts — and the six fields become eight bytes each: `CellSpans` 92 → 68 B, `CellSlot` 100 → 76 B, the slot block **10 001 000 → 7 600 760**. The same idiom `Target.no_fref` uses one layer up. **Result (§9.1): first-recalc RSS 69.5 → 68.5 MiB — 4.5× the 15.15 MiB ceiling, from 4.6×.** Peak live 78 087 308 → **76 794 991, which is the drive's publish, unmoved to the byte** — the budget spent exactly, for the third row running. **The finding is the leftover: the cut removed 2 400 240 B and only 1 292 317 of them were purchasable**, so 1.1 MB of real reduction bought nothing, and staging's plateau now sits 1 107 923 B *below* the peak with its retained cells (13 601 360), its slots (7 600 760) and the staged set (6 400 000) all still resident and all worth zero RSS. Two rows in a row have ended with the loser's mass intact; the next row is the drive's publish era again — where M10m's protected blocks (evaluate store 6 400 000, iterate `reports` 5 759 936, graph residual 5 587 480) have finally become budget — or the ceiling conversation, which at 4.5× and fourteen rows in is overdue | Fixtures **and** saved archives byte-identical to the M10m baseline across all four workloads + f1 small/tiny — 12/12; **`compare_bench --gate` exit 0 in both directions**, all 30 lanes ok either way; the same-binary control was taken before believing any lane and this window was quiet (F1 named eval +0.2 % on the identical binary, no lane flagged), so the two brackets against the base (−1.4 % and +0.6 % on F1 named eval) straddle a neutral change — named eval 387.6–395.3 ms, the 500 ms ceiling stays met; zig build + zig build test green (9870 tests, +20); zig fmt --check clean; the RSS probe byte-reproducible across three runs (73 596 928 B each), and the M10m binary re-measured in the same window at 74 711 040 B each |
| **M10o** ✅ | **The drive-publish row — the block that spans the whole curve** (`feat/m10o-cell-extras`). The 68.5 MiB re-profiles at the M10n baseline first and reproduces it exactly (RSS 73 596 928 B ×3; peak live 76 794 991; every era byte-for-byte). **The pre-flight read corrected the row's premise before a byte was cut.** The row was scoped around the four blocks M10m protected and M10n freed — the evaluate store 6 400 000, iterate `reports` 5 759 936, the graph build residual 5 587 480, the graph's linked edges 10 210 051 — priced against the plan era at 72 740 090, a budget of ~4.05 MB. The trace says 1 107 923 B. **Staging's plateau (75 687 068) holds none of the four either**, so it is the first era a drive-exclusive cut fails to reach, and it takes the peak the moment they go: 29 436 199 B of mass standing on 1 107 923 B of budget. Applying M10n's rule *before* cutting is what the rule is for. The row was won instead by a block none of the four is. **`WorkbookEnv.Cell` is the run's most-multiplied record** — one per coordinate per layer, held in 64-entry chunks, and the drive publishes a *second* entry at every formula cell, so the named workload carries 100 010 of them into the drive and 180 010 out of it (every full chunk splits exactly once; fill lands at 57.6/64). Four of its ten fields — `cm`, `vm`, `array_ref`, `spill_anchor` — are 52 of its 136 bytes and are zero-or-null on every cell of every §9 workload: cell metadata, a legacy CSE array's declared range, a spill tail's owning anchor. **They move behind a `u32` index into a model-arena `extras` list** (`Cell.no_extra` = `maxInt(u32)`, the idiom `Target.no_fref` and M10n's `OptSpan` already use), and a cell that carries none reads the all-default record without branching. `Cell` **136 → 88 B**, `Chunk` 8712 → 5640 B, the sheet directory's chunk bytes **27 233 712 → 17 630 640** at the drive's peak instant. The width is a test: a byte added there costs 180 KB. **Result (§9.1): first-recalc RSS 68.5 → 59.3 MiB — 3.9× the 15.15 MiB ceiling, from 4.5×, the first sub-4× reading the ladder has produced** and 9 617 408 B in one row against 2–3.5 MB for each of its five predecessors. Peak live 76 794 991 → **65 181 445**, still the drive's publish. **The finding is why it is nine megabytes and not two: a phase-local block's budget is bounded by the next phase; a record held across every phase is bounded only by the phases that predate it.** The model's cell record is live continuously from era 12 to era 23, so the first era it does not reach is era 10 — the decode phase, 52 684 820, measured identical before and after — and the budget is **24 110 171 B, the largest any row in the series has had**. 11 613 546 of it was spent; what stopped the row was the record, not the budget. (Second-order, and M10e's mechanism paying out for once: chunk *demand* fell 9 603 072 B and the model arena returned 11 611 854, because a shrinking demand curve crosses a ladder rung on the way down.) The peak is the drive's publish again with staging's plateau 1 107 923 B behind it, and the six drive-exclusive blocks that could collect it are each floored by a contract this row may not move — `Report.components` is public and read by twenty tests, `CellInput.array` is an input-API shape, `Key.Producer`'s two spellings set the canonical producer order, `Ref` and the graph block are floored by `Key.Span`, and the journal is 27 935 B short even if it were free | Fixtures **and** saved archives byte-identical to the M10n baseline across all four workloads × tiny/small/named — 24/24; **`compare_bench --gate` exit 0 in both directions**, all 30 lanes ok either way; the same-binary control was taken first and the first attempt at it flagged a lane at +29.9 % — a concurrent `zig build test`, not the binary, which is the lesson M10k's window-drift rule exists to catch — so the window was cleared and re-taken, and the identical binary then reproduced within ±1 % on every substantial lane (F1 named eval −0.0 %, criteria-eval small −0.1 %); against that control the base bracket is −0.7 % on F1 named eval, **378.5 ms, the 500 ms ceiling stays met**; zig build + zig build test green (9878 tests, +8); zig fmt --check clean; the RSS probe reproducible across three runs (63 979 520 B), and the M10n binary re-measured in the same window at 73 596 928 B each |
| **M10p** ✅ | **The model-value row — the cut the era trace could not see** (`feat/m10p-cell-value`). The 59.3 MiB re-profiles at the M10o baseline first and reproduces it exactly: peak live 65 181 445 to the byte, every era byte-for-byte, RSS ×3 stable. (The raw `/usr/bin/time -l` figure read 63 963 136 rather than the recorded 63 979 520 — one 16 KiB page — but the usage-invocation baseline was low by the *same* page, so the baseline-adjusted number, which is what §9.1 records, reproduced exactly at **62 160 896 B**. A page of argv and environment is not a regression, and the adjusted figure is immune to it.) The cut is the one M10o's closing line named: **`WorkbookEnv.Cell.v` was `?ScalarValue`, 40 bytes to carry 32.** `ScalarValue`'s payload is eight-aligned, so the optional's presence flag cannot share the union's tag word and opens a new one; absence becomes a sixth case of that tag instead (`OptValue.absent`, the idiom `Target.no_fref` and M10n's `OptSpan` already use), and `Cell` goes **88 → 80 B**, `Chunk` 5640 → 5128 B, the sheet directory's chunk bytes 17 630 640 → **16 030 128** at the drive's peak instant. `absent` is not `blank` and the type keeps them apart — `get()` returns the `?ScalarValue`, `orBlank()` performs the merged view's collapse at the read site where §5.6f wants it, and a test pins the distinction alongside the width. **Result (§9.1): first-recalc RSS 59.3 → 57.8 MiB — 3.8× the 15.15 MiB ceiling, from 3.9×**; baseline-adjusted 62 160 896 → **60 571 648 B**, −1 589 248. **The finding is that peak live did not move: 65 181 445 → 65 181 995, up 550 bytes, while RSS fell 1 572 864.** Eras 12–19 fell by exactly 6 814 and eras 20–23 *rose* by exactly 550; in the era trace's own currency this row bought nothing at the peak. The records are arena-held, and the model arena's ×1.5 chunk ladder did not step down — the 1 600 512 B came out of the *fill* inside chunks already mapped, and a mapped page that is never written is not resident. **The heap profiler prices the ladder; RSS prices the fill.** M10o's second-order note was the same mechanism with the sign flipped (there the demand curve crossed a rung and the arena returned *more* than the cut). So the era trace's budget — 4 732 804 B for a shared cut, floored by era 15 — is denominated in peak-live bytes, and for an arena-held record it can understate the RSS available: this row spent **zero** of that budget by the trace's reckoning and still collected 1.57 MB. The next row is the ceiling conversation, which at 3.8× and sixteen rows in is well past due | Fixtures **and** saved archives byte-identical to the M10o baseline across all four workloads × tiny/small/named — **24/24**; **`compare_bench --gate` exit 0 in both directions**, all 30 lanes ok either way, with the same-binary control taken **first** in a quiet window per M10k/M10o's drift rule — the control came back clean on every substantial lane (F1 named eval +1.2 %, criteria-eval small −0.9 %, text-eval small +0.7 %), so the bracket is trustworthy: F1 named eval **386.72 → 375.09 ms, −3.0 % (z −3.2), the 500 ms ceiling stays met**, named `save` −0.4 %, criteria/text/registry eval flat-to-faster; `zig build` + `zig build test` green (132/132 steps, **9882 tests, +4**); `zig fmt --check` clean; the RSS probe byte-reproducible across three runs (62 390 272 B) |
| **M10r** ✅ | **The in-place-publication row — the drive stops writing a second `Cell` at every formula cell** (`feat/m10r-inplace-publish`). The 57.8 MiB re-profiles at the M10q baseline first and reproduces it exactly: peak live **65 181 995**, eras 10 = 52 684 820 / 15 = 60 441 827 / 20 = 65 181 995 / 21 = 64 074 072 byte-for-byte, RSS 62 439 424 B ×3, and the fill probe's model arena at capacity_end 22 842 794 / fill_peak 18 157 309 / unfilled 4 685 485. (That baseline is **M10q's**, not M10p's — M10q compiled the probe in at a measured 32 768 B raw, the usage-invocation baseline rose by the other 16 KiB page, and the adjusted figure moved 60 571 648 → **60 588 032 B**, 57.77 → 57.78 MiB. Restated here because M10q carries no ladder row.) **The cut is the one `Cell`'s own header has named since M10o**: the drive published a second record at every formula cell, and the two were never both read — the computed one shadowed the stored one for every merged read, and the stored one answered `formulaAt`, `dialectOf` and `svOccupied` out of `formula_text`, `cache` and `extra`, which a computed record does not carry. Publication now overwrites `v` on the record already there. **It cannot be staged**: the engine's journal holds only a `CellRef`, so a refusal can uncover a lower layer but never restore an overwritten one — `iterate.Options.retraction` gains `.candidate_discard` (no journal, no retraction), `recalc_run` is its one caller because its `WorkbookEnv` is per-run and dies at `defer model.deinit()` on every path while the live workbook is touched only at `Candidate.swap`, `journal` stays the default for the generic `Host`, and `retractResult` asserts the pairing. Three exclusions, each with a test that fails without it: never a spill tail, a formula record only, and never a published `.blank` (`dupeValue` folds blank to `.absent`, which on a record carrying `formula_text` is this type's spelling of *uncached*). **Result (§9.1): first-recalc RSS 57.8 → 52.3 MiB — 3.5× the 15.15 MiB ceiling, from 3.8×**; baseline-adjusted 60 588 032 → **54 820 864 B**, −5 767 168. Peak live 65 181 995 → **60 441 827**, and eras 10 and 15 are byte-identical across the cut, so **the peak moves to era 15, the graph build** — exactly where goal_codex §2 priced it. Era 20 loses the three allocations the pre-flight named (`id82` 7 720 116, the publication's arena chunk; `id68` 1 079 988, the journal's reserve; `id21`'s publication step of 7 878 — 8 807 982 against 8 806 210 measured). **The finding: a cut can pay MORE than its peak-live budget.** Peak live fell 4 740 168 — the era-15 cap, spent to the byte — and RSS fell **1 027 000 past it**, because RSS sat 2 742 571 B below peak live before and 3 769 571 B after: the removed bytes were touched fill inside chunks whose ×1.5 ladder never stepped, so their pages left residency while the era heights holding them did not move. M10q's probe reads it directly — model `fill_peak` 18 157 309 → **10 122 389** (−8 034 920, against 8 015 064 predicted from 1 563 × 5 128) while `capacity_end` fell only 7 727 970 and **unfilled rose**, 4 685 485 → 4 992 435. M10p spent **zero** of its budget and collected 1.57 MB; this row spent its budget **exactly** and collected 1.03 MB past it — between them, **the peak-live budget bounds what the era trace will show and bounds RSS in neither direction**. The next row is goal_codex §6's columnar model, the only scheduled proposal that reaches era 15 — or the ceiling conversation, still overdue at 3.5× | Fixtures **and** saved archives byte-identical to the M10q baseline across all four workloads × tiny/small/named — **24/24**; the RSS probe byte-reproducible across three runs (56 672 256 B), with the usage-invocation baseline identical on both binaries (1 851 392 B), so the adjusted figure carries no page shift this time; `zig build` + `zig build test` green (132/132 steps, **9932 tests, +18**); `zig fmt --check` clean; **`compare_bench --gate` exit 0 in BOTH directions, all 30 lanes ok either way**, with the same-binary control taken **first** in a quiet window per M10k/M10o's drift rule — and it is the tightest control this ladder has recorded, every substantial lane inside ±0.4 % (only the sub-2.5 ms `open` lanes swung, to ±4.6 %, under the gate's 0.5 ms absolute floor), so the bracket is trustworthy: F1 named eval **535.17 → 517.47 ms, −3.3 %**, named `save` −2.6 %, and every other substantial lane faster too (criteria-eval small −0.8 % / tiny −3.8 %, registry-eval small −2.8 %, text-eval small −2.5 %, f1 small −2.4 %, f1 tiny −1.5 %). **The 500 ms evaluate ceiling was NOT measurable in this window, and this row does not claim it.** The host runs ~1.45× the session that set the 375–433 ms band, and the attribution is direct rather than assumed: M10p's own commit (`9127713`) was rebuilt and measured beside this one, four binaries interleaved in one window over three rounds with the order reversed in round 2, and reads **542.65 ms** against the 375.09 ms its own row recorded — with `611f5c7` at 541.34 and main at 543.38, three commits inside 2.0 ms. A 15-minute idle cooldown moved it to 540.40 (cut 522.86), so it is host state, not thermal soak, and not code. Anchored to M10p's binary in the same window the cut is **540.40 → 522.86 ms, −3.2 %**, which puts its M10p-equivalent at **≈362.9 ms** — recorded as derived, not as a measurement, and a clean-host re-measure of the absolute ceiling is owed |
| **M10s** ✅ | **The model-record row — the trace over-reports, the fill probe does not** (`feat/m10s-columnar-model`). The 52.3 MiB re-profiles at the M10r baseline first and reproduces it exactly: peak live **60 441 827**, eras 10 = 52 684 820 / 15 = 60 441 827 / 20 = 56 375 785 / 21 = 56 346 078 byte-for-byte, and the fill probe's model arena at capacity_end 15 114 824 / fill_peak 10 122 389 / unfilled 4 992 435 — every figure exact. RSS read 56 688 640 ×3 against M10r's recorded 56 672 256: **one 16 KiB page**, on a host whose own usage-invocation baseline has moved 589 824 B since that session (`/usr/bin/time -l` on `true` reads 1 261 568 here against 1 851 392 there), which is the page-sensitivity M10p logged; the deltas below are measured against the figure reproduced here, same build recipe both sides. **The row took goal_codex §6's narrowing branch, not its structure-of-arrays branch, and is named for what it did** — nothing was made columnar, and every order-is-the-contract reader (`merged`, `advance`, `lowerBound`, `at`, `next`, the range iterators, `vtDialectOf`, `svOccupied`, `clearTailsIn`, the spill host, and M10r's `inPlaceTarget`, whose caller writes through the returned `Sheet.Cursor`) was re-checked and needed no change, because the record stayed a record at a different width. **The cut: `Cell.cache` was a `graph.CacheState`, 16 bytes to carry a tag and an f64 the record already held.** `v` and `cache` are both derived from the same `decode.InputCell` at the one site that builds a stored cell, so a `.number` cache is always the `.number` in `v` — the same number, not merely the same type. The tag stays at one byte because the tag is *not* derivable: `.absent` is what both a missing `<v>` and a genuine blank read as, and §5.6c's seed table has to tell those apart from a cached value. `cacheState()` reassembles it; `Cell` **80 → 64 B**, `Chunk` 5128 → 4104 B. Two writers keep the pairing and there are no others — decode sets both from one `InputCell`, and M10r's in-place `putComputed` re-tags as it overwrites `v`, which keeps an invariant true rather than fixing an observable bug since `buildInput` is `cache`'s only reader and has already run by then. **Result (§9.1): first-recalc RSS 52.3 → 50.8 MiB — 3.35× the 15.15 MiB ceiling, from 3.46×**; baseline-adjusted 54 886 400 → **53 280 768 B**, −1 605 632. Peak live 60 441 827 → **55 311 851**, and the peak is **still era 15**, with era 10 (decode) byte-identical at 52 684 820 and now 2 627 031 B below it — neither park condition fired. **The finding: the trace over-reported by 3.2×, and RSS took the fill.** This is M10p's experiment run again with the same fill delta and the opposite trace outcome. Gross fill demand removed was **1 600 512 B, predicted to the byte** (1563 chunks × 1024): model `fill_peak` 10 122 389 → 8 521 877 and `handed` 10 150 853 → 8 550 341, both −1 600 512. But `capacity_end` fell **5 129 952** (15 114 824 → 9 984 872, showing as `id21` 12 330 726 → 7 201 034 plus `id20` −284) — 3.2× the bytes actually freed, because the ×1.5 ladder stepped down a generation and `unfilled` fell 4 992 435 → 1 462 995. The identity is exact: **backing 5 129 952 = fill 1 600 512 + unfilled 3 529 440**. A mapped page never written is never resident, so that 3 529 440 cost nothing to give up and earned nothing back — RSS moved −1 622 016. Across the pair, M10p (trace 0, RSS −1 572 864) and M10s (trace −5 129 976, RSS −1 622 016), the fill deltas are identical to the byte, the RSS deltas agree within 3 %, and the trace deltas differ by 5.1 MB; baseline-adjusted, RSS falls 1 605 632 against 1 600 512 of fill, **0.32 %**. M10r settled that the peak-live budget bounds RSS in neither direction; this row says which instrument does bind — **the era trace prices the ladder, the fill probe prices the pages, and it is the pages that are resident.** Price a row by the fill probe. Second-order, and larger than the control's noise: the narrower record is *faster* on the whole-column-scan lanes, criteria-eval −8.2 % and criteria-save −7.9 % (z −22.6) against a ±3.7 % control — 20 % less memory traffic per chunk scanned. The SoA half of §6 is untouched and still available, now floored at 2 627 031 B of peak live by era 10 | Fixtures **and** saved archives byte-identical to the M10r baseline across all four workloads × tiny/small/named — **24/24**; the RSS probe byte-reproducible across three runs (55 066 624 B); `zig build` + `zig build test` green (132/132 steps, **9941 tests, +9**); `zig fmt --check` clean; **`compare_bench --gate` exit 0 in BOTH directions, all 30 lanes ok either way**, with the same-binary control taken **first** in a quiet window per M10k/M10o's drift rule — the control was clean on every substantial lane (F1 named eval +0.5 %, criteria-eval small +1.0 %, registry-eval small +0.4 %, text-eval small +1.4 %; only the sub-2.5 ms `open` lanes swung, to +19.6 %, under the gate's 0.5 ms absolute floor), so the bracket is trustworthy: F1 named eval **363.70 → 356.82 ms, −1.9 %**, named `save` −1.7 %, and every substantial lane faster. **The 500 ms evaluate ceiling is measurable again in this window, and this row claims it.** M10r's condition was met head-on: M10p's own commit (`9127713`) was rebuilt and measured beside this one and reads **373.02 ms against the 375.09 ms its own row recorded** — inside its band, −0.55 %, where M10r's session read it at 542.65. The ~1.45× host state M10r documented is gone, the clean-host re-measure M10r said was owed is hereby taken, and M10r's *derived* M10p-equivalent of ≈362.9 ms is confirmed by direct measurement of its binary here at 363.70 ms — a 0.2 % match that retroactively validates its anchoring method |
...
> re-affirmed by M10u (§9.1d) and M10v (§9.1e), ratcheted to
> **51 645 808 B** by M10w (§9.1f) and to **50 189 598 B** by M10x
> (§9.1g) — and 15.15 MiB survives as a non-blocking research hypothesis
> expressed per modeled cell (158.8 B/cell against 497 B/cell measured in
> the ReleaseSafe gate lane, 496 B/cell in ReleaseFast — §9.1g). The
> evaluate ≤ 500 ms and end-to-end ≤ 1 s ceilings are **unchanged** by
> this amendment.
>
...
**M10f — the third staging row (recorded 2026-08-09).** Same
discipline: attribute the peak, cut what the attribution names, name
what remains. The probe is unchanged.

**What held the recorded 122.9 MiB** was M10e's closing table,
reproduced to the site at re-profiling (the RSS lane re-measured
130 727 936 B on main — one page above the recorded run). The exact
byte counts under two of its lines sharpen on inspection: the
"publications list" is 80 000 records at 120 B, and the "Target
array" was an 8.32 MB array (104 B × 80 000) minted as a 12.48 MiB
chunk — a fresh arena's FIRST allocation is also sized at 1.5× the
request, so the chunk-ladder mechanism M10e named needs no prior rung
to overshoot.

| Record | was | is | how |
|---|---:|---:|---|
| the scan's slots and rows | dupes into the scan arena — the 10.0 MB slot dupe (100 B × 100 010) minted the 16.4 MiB half-again chunk | **exact gpa blocks** | `gpa.dupe` at scan end, freed at `deinit` — the projection borrows both, so they outlive `releaseCells`; only the decoded strings, whose total no count predicts, stay laddered |
| the `Target` record | 104 B, embedding its publication's result, shape, role and dialect | **48 B, naming it: `pub_idx` into the borrowed staged set** | the publication exists once; the list stays alive until the plan has rendered every replacement into its own arena, still freed before the splice; the target chunk's mint drops 12.48 → 5.76 MB |
| the drive's component report | duped at the drive's very peak — 5.76 MB minted beside the list's own 6.8 MB ladder buffer, resident through staging for three scalars | **moved** (`toOwnedSlice`), the list **pre-sized exact** to the condensation's component count, folded to its three scalars and freed before staging | the exact pre-size makes the move free (lengths agree) and retires the append ladder's churn — this is a count, not a bound, so it lays no floor under the early-evaluation transients (the M10e +3.3 MB answer is about the maps, and stands) |

**The row's finding.** The staging cuts alone moved `/usr/bin/time -l`
by nothing: the projection instant fell from 118.5 to ~104 MiB live
and RSS held to the page (130 727 936 → 130 760 704 B). §9.1's RSS
was already pinned by the drive's era at M10e's close — the drive's
end (graph block + capture arena + per-pass maps + run arena + report
mint + model) had mapped the same height the staging instant occupied.
"The peak instant returns to staging" was true of the live-byte
profile and false of the pages; cutting the drive-end instant (the
report move) is what moved the number.

| | M10e (recorded above) | **M10f** |
|---|---:|---:|
| profile site-sum at the peak instant | 118.5 MiB (the projection) | **107.3 MiB (the drive's end)** |
| `/usr/bin/time -l` peak | 130 695 168 B | **126 795 776 B** |
| process baseline | 1.75 MiB | 1.78 MiB |
| baseline-adjusted | **122.9 MiB** | **119.1 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 8.1× | **7.9×** |

The four `--gate` lanes re-ran against main in the same session and
stayed green, every delta inside the noise band — F1 named eval
−2.0 % (416.5 ms median as recalc − open; the 500 ms ceiling stays
met), named `save` −2.9 %, criteria eval +0.3 %, registry eval
−0.5 %, text eval +1.4 % — the restructure allocates less, and all
four workloads' saved archives are byte-identical to main's.

The restructure also exposed a latent use-after-free in a §5.8c test
helper: `evaluateReopened` returned a scalar whose text borrowed the
helper's own scan arena, freed at return — it passed for as long as
the allocator happened to keep the page mapped, and the slots/rows
re-homing changed the layout enough to unmap it. The helper now runs
its assertions while the memory lives.

**What remains at the peak — which is the drive's end now.** Model +
computed + store at 40.5 MiB (the workbook's own data), the graph's
exact block (12.8) and its capture arena (6.2), the per-pass maps
(~27 pooled — M10e's measured answer stands), the run arena's payload
dupes (9.8), the report block (5.5, exact) and the published list
(5.5). Above the live sum sits ~13 MiB the allocator retains against
it — rehash and ladder churn from the first pass, pages §9.1's RSS
counts and no site owns. Cutting further means the maps' pooled
capacities or that churn — or renegotiating the ceiling; the
patcher's side of the ledger now matches the engine's: exact blocks,
counted lifetimes.

**M10g — the drive-era row (recorded 2026-08-09).** Same discipline:
attribute the peak, cut what the attribution names, name what remains.
The probe is unchanged.

**What held the recorded 119.1 MiB** was M10f's closing table,
reproduced to the site at re-profiling (the RSS lane re-measured
126 779 392 B on main — one page under the recorded run; peak live
107.3 MiB at the drive's end). On inspection, the drive's three big
per-pass records are **write-only on the common path** — each has
exactly one reader, and none of the three readers is reachable on a
workbook without iteration or dynamic references:

| Record | at the peak | its one reader | now |
|---|---:|---|---|
| `touched` | 8.1 MiB + 15.3 MB churn | none — M10b's gate restructure left the list behind; only its sibling set was ever read | **deleted** |
| `held` | 6.6 MiB + 13.9 MB churn | `heldValue`, whose one caller sits behind `iterateComponent`'s `settings.iterate` refusal | recorded **only under iteration** |
| `touched_set` | 6.1 MiB + 12.8 MB churn | `foldedPassEdges`' carry, a walk over `dynamic.items` — empty without dynamic references | recorded **only while `dynamic` holds edges** |

(the `scope` set also pre-sizes exact from the plan's component list —
a count, not a bound, sized where it is filled, so it lays no floor
under the transients M10e's reverted pre-sizing warned about). The
cut moved RSS to 120 455 168 B and moved the **peak instant upstream
into the graph build** (103.1 MiB live) — the first time an instant
before the drive's end set the mark. What the new attribution named
there: `condensationOrder`'s per-component `feeds` lists — ~80 000
first blocks at the allocator's cache-line floor (32 × u32), 15.2 MB
live at the peak instant for ~400 KB of edge payload — and the
build's three append ladders (`owners` churned 30.7 MB for a 10.2 MB
list, `keys` 15.3 for 5.6, `logs` 5.9 for 3.1). The feeds became one
flat (d, c) pair list counting-sorted into a CSR carve; the ladders
pre-size from counts known before their loops (owners = the input's
three lengths, keys = owners + deduped tails + one arithmetic walk
over `refs`, logs in lockstep with `refs`).

| | M10f (recorded above) | **M10g** |
|---|---:|---:|
| profile site-sum at the peak instant | 107.3 MiB (the drive's end) | **100.1 MiB (staging's scan)** |
| `/usr/bin/time -l` peak | 126 795 776 B | **100 024 320 B** |
| process baseline | 1.78 MiB | 1.78 MiB |
| baseline-adjusted | **119.1 MiB** | **93.6 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 7.9× | **6.2×** |

**The churn gap inverted.** M10f closed with ~13 MiB the allocator
retained over the live sum — rehash and ladder churn from the first
pass, pages no site owned. Those pages were the maps' and the build's
growth ladders, and they left with their owners: RSS now sits ~6 MiB
*below* the profile's site-sum, because what remains is exact blocks
whose capacity tails are allocated but never faulted. The gap was
never allocator behavior a record restructure couldn't reach — it was
the ladders' freed rungs, and cutting the rungs cut the pages.

The lanes re-ran against main in the same session and the four
workloads *improved* — the retired reallocs were wall-clock too: F1
named eval −5.0 % (391.9 ms median as recalc − open; the 500 ms
ceiling stays met), named `save` −5.9 %, criteria eval −0.1 %,
registry eval −4.6 %, text eval −5.3 % — and all four workloads'
saved archives are byte-identical to main's.

**What remains at the peak — which is staging's scan again.** Model +
computed + store at 40.5 MiB (the workbook's own data), the scan's
arena under `scanSheet` (24.3 MiB at the peak instant, 76.4 MB
churned — the decoded strings M10f left laddered because no count
predicts their total), the scan's exact slot-and-row blocks (13.0),
the run arena's payload dupes (9.8), the published list (4.3) and its
...
**M10i — the record-lists row (recorded 2026-08-10).** Same
discipline: attribute the peak, cut what the attribution names, name
what remains. The probe is unchanged.

**What held the recorded 90.1 MiB** was M10h's closing table,
reproduced exactly at re-profiling on all three probes: RSS
96 337 920 B across three runs — the recorded bytes; peak live
101 145 606; the shortlisted record-list site byte-for-byte
(25 514 508 at peak, 59 allocs, 76 434 388 churned), the cells'
13 601 360 exact dupe beside it. A Debug-granularity run de-inlines
the shortlisted site into the three lists M10h named — `cells`
14 511 880 / 22 allocs / 43 487 632 churned, `slots` 10 670 500 / 23 /
31 985 700, `rows` 332 128 / 14 / 961 056 — the attribution confirmed
before anything was cut.

**The finding: the copy instant was never the peak.** The shortlist's
own cut proved it. Handing each growing list off in its own block —
`toOwnedSlice` remaps to length in place, because the lists are
page-backed far over `SmpAllocator`'s slab line and the shrink
munmaps the doubling tail — removed the copy instant entirely (the
exact dupes are never allocated; the dupe-and-free dated from the
lists' arena era, M10c, and an arena cannot shrink where a `gpa`
block can), and `peak_live_bytes` did not move by one byte. The true
peak was the projection instant, tied at the same 101 145 606 inside
the profiler's 1 MiB snapshot-step blind spot — the at-peak table
named the copy instant only because the later instant never exceeded
it by the snapshot step. What the projection instant held, and §9.1
now cuts, is 16.9 MB nothing would ever read again:

| dead weight at the projection instant | bytes | died at | dies at now |
|---|---:|---|---|
| the bridge's input cells | 6 400 000 | `prepare`'s end (`defer`) | the drive's return |
| the roots | 3 840 000 | `prepare`'s end (`defer`) | the drive's return |
| the driver's published list | 4 480 000 | `driver.deinit` | its last read — the final published sheet's publication build |
| the published index | 2 228 248 | `driver.deinit` | same |

M10f's fold-before-staging pattern, two more seams. The counted
pre-walk was NOT taken, and the reason is a measurement: with the
dupes gone the whole scan era sits below the peak, so exactness there
buys zero RSS — and the ladders' 76.4 MB of churn already returns to
the OS (the probe reads 7.3 MiB *below* the live site-sum, M10g's
inverted gap again).

| | M10h (recorded above) | **M10i** |
|---|---:|---:|
| profile site-sum at the peak instant | 96.5 MiB (staging's scan) | **88.7 MiB (the graph build)** |
| `/usr/bin/time -l` peak | 96 337 920 B | **85 721 088 B** |
| process baseline | 1.78 MiB | 1.73 MiB |
| baseline-adjusted | **90.1 MiB** | **80.0 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 5.9× | **5.3×** |

The lanes re-ran against main in the same session and the gate is
green across all four workloads — F1 named eval +5.3 % lane median
(415.6 ms as recalc − open; the 500 ms ceiling stays met — and an
interleaved head-to-head on the same two binaries read 402.9 ± 3.0 vs
403.9 ± 5.0 ms, parity, naming the lane delta sequential-run drift),
named `save` −2.2 %, criteria eval −7.1 %, text eval −8.8 %, registry
eval −9.7 % (largest z 4.0 on criteria-eval tiny at +3.2 %, under the
gate's compound threshold) — and all four workloads' saved archives
are byte-identical to main's.

**What remains at the peak — which is the graph build now.** The
patcher no longer sets the mark; `engine.graph.build` does, before
the drive, where the computed layer does not yet exist: the link
block (13 450 007), the CSR edge pair (12 205 998), the owner map
(7 680 000), the capture arena (6 551 868 across 17 chunks), keys +
captures (5 599 952), condensation scratch (4 257 336 across 26),
over the pre-drive base — model 20 065 940, part bytes 7 951 114, the
bridge's input records 6 400 000, shared texts 2 780 178. These are
M10g's blocks: already exact, already sized at link entry, holding
~23 MB of records the drive needs. Cutting further means shrinking
the graph's records themselves — or renegotiating the ceiling, which
at 5.3× is the conversation the ladder has been walking toward.

**M10j — the graph row (recorded 2026-08-10).** Same discipline:
attribute the peak, cut what the attribution names, name what
remains. The probe is unchanged.

**What held the recorded 80.0 MiB** was M10i's closing table,
reproduced exactly at re-profiling on all three probes: RSS
85 721 088 B across three runs — the recorded bytes; peak live
93 003 295; every shortlisted site byte-for-byte. A
Debug-granularity run de-inlines the two sites ReleaseSafe had left
merged and corrects one of the shortlist's names. The "CSR edge
pair" was not retained records at all: it was the edge-accumulation
**scratch** — per-node adjacency lists growing inside a scratch
arena, 15 449 982 B at the peak instant (two growth chunks at
12 205 998, plus the 3 243 984 chunk the 24 B × 89 999 list-header
array minted) held against **398 744 B of admitted edges** — dead
the moment the exact CSR copy ends, `defer`-held through Tarjan and
the condensation. The era's other mass was the builder's own lists
past their last read — the owner map (7 680 000, dead after the
walk-log loop), the keys' exact block (4 319 952, dead after the
block dupe), the refs headers (1 280 000) and logs (960 000) — and
the capture arena's 17-chunk ladder (6 551 868 held for 5 587 480
of refs payload).

**The cuts, round one — the graph's own records** (output bytes
unchanged, verified by digest): (1) edges accumulate as one flat
admitted-pair list plus a per-node degree count, carved into the
exact CSR block by a counting sort — condensationOrder's own shape
(M10g) — with a per-run `seen` list answering exactly the
membership the per-node list answered, because a node's edges
arrive in one contiguous run and the two loops' node kinds are
disjoint: same admits, same charges, same `stats.edges`; (2) every
builder list dies at its last read (M10f's fold pattern, five more
seams); (3) the capture arena is retired — `takeInto` appends each
owner's refs to one flat gpa list, and `link` takes it as the
graph's exact refs block (`toOwnedSlice`; the graph now carries
`refs_data`, not an arena).

**The row's finding: the graph era was never the pin.** The cuts
above moved peak live 93 003 295 → 90 958 663 — and
`/usr/bin/time -l` moved sixteen kilobytes. The drive era had
mapped within 2.0 MB of the graph era's height, so §9.1's number
was the drive's all along (M10f's finding, one era later: cutting
an era that is not the pin moves nothing). The at-peak table, now
naming the drive's publish instant, held its own dead weight:

| dead weight at the drive's peak instant | bytes | died at | dies at now |
|---|---:|---|---|
| `prepare`'s pre-flight plan — discarded at the call site | 1 260 124 | `prepare`'s end (arena) | the call it is discarded at (own scratch) |
| `planScope`'s plan — read once, for the scope set | 1 260 124 | `driver.deinit` (engine arena) | the scope fill (own scratch) |
| twelve slice-header bytes per node in the walk logs | 1 079 988 | — | `WalkLog` records a span of the refs block, five u32s (32 → 20 B; the link block 13 450 007 → 12 370 019, and the Δ names the node count: n = 89 999) |

**Result.** Same methodology (ReleaseSafe — the flag is
load-bearing — `/usr/bin/time -l`, usage-invocation baseline, first
recalc, digest-gated fixture):

| | M10i (recorded above) | **M10j** |
|---|---:|---:|
| profile site-sum at the peak instant | 88.7 MiB (the graph build) | **82.4 MiB (the drive's publish)** |
| `/usr/bin/time -l` peak | 85 721 088 B | **83 296 256 B** |
| process baseline | 1.73 MiB | 1.73 MiB |
| baseline-adjusted | 80.0 MiB | **77.7 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 5.3× | **5.1×** |

The lanes re-ran against main in the same session and the gate is
green across all four workloads — F1 named eval −0.5 % (391.19 ms
median; the 500 ms ceiling stays met), named `save` −0.8 %,
criteria, text and registry eval all improved or flat (largest z
1.3, registry-open tiny at +23.4 % on a 1.6 ms lane, under the
gate's compound threshold) — and all four workloads' saved archives
are byte-identical to main's (+ f1 small/tiny).

**What remains at the peak — which is the drive's publish now.**
The workbook's own data (model 20 065 940, computed-so-far
11 603 808, part bytes 7 951 114, shared texts 2 780 178); the
graph the drive is reading (the link block 12 370 019, the refs
block 5 587 480, edge data 398 744); the bridge's input records and
roots (10 240 000 — dead at the drive's return since M10i, alive
under the instant by construction); and the drive's own contract
(the component reports at 64 B × 89 999 = 5 759 936, the published
list 4 480 000 and its index 2 228 248, the publication build's
ladder 1 493 928). Cutting further means the drive's records — the
report row, the published pair, the graph block's remaining slice
headers (deps and order, 16 B × n each, CSR-able for ~2.9 MB) — or
renegotiating the ceiling, which at 5.1× is the conversation the
ladder has been walking toward.

**M10k — the drive-records row (recorded 2026-08-10).** Same
discipline — attribute the peak, cut what the attribution names, name
what remains — plus one new instrument. The probe is otherwise
unchanged, and the M10j baseline reproduced exactly before any cut:
RSS 83 296 256 B ×3, peak live 86 398 295, every shortlisted site
byte-for-byte.

**The era trace.** M10j learned post-mortem that cutting an era that
is not the pin moves nothing; the profiler now records every era's
local maximum (2 MiB dip threshold) with a lagging per-site snapshot,
so the runner-up question is answered *before* the cut. The pre-flight
read: the runner-up sat **1.8 MB under the peak** — the staging
plateau, whose splice-scan instant (84 563 635) and projection instant
(82 457 168) hold the published pair but none of the drive's other
records. Three consequences, all applied before cutting: the published
pair pays on both sides of the era boundary; drive-exclusive cuts pay
only the 1.8 MB gap; and **narrowing ComponentReport pays zero** —
pkg folds it to three scalars before staging (M10f's fold), and once
the gap is covered the publish instant sits below the plateau. The
shortlisted report row was deliberately not cut.

**The cuts** (output bytes unchanged — fixtures and saved archives
byte-identical to main across all four workloads plus f1 small/tiny):

| record | before | after | mechanism |
|---|---:|---:|---|
| `Published` (80 000 entries) | 4 480 000 (56 B) | 3 200 000 (40 B) | the `ScalarValue` field was two nested union tags and their padding around a payload never wider than a pointer and a length — flattened to tag + word + extra |
| `published_at` | 2 228 248 | 1 703 960 | key packed to u64: row < 2²¹ and col < 2¹⁴ are the format's own bounds, 29 bits of sheet remain |
| the graph link block | 12 370 019 | 10 210 051 | deps/order slice headers → CSR offsets; the fat headers Tarjan and the condensation walk are link scratch now |
| the engine journal | 1 493 928 / 18 chunks | 1 079 988 / 1 | reserved to the node count exactly |
| the scope set | 655 384 | ~11 KB | u32 membership map → one bit per component |
...
| | M10j (recorded above) | **M10k** |
|---|---:|---:|
| peak live at the peak instant | 86 398 295 (the drive's publish) | **82 759 347 (staging's splice scan)** |
| `/usr/bin/time -l` peak | 83 296 256 B | **79 577 088 B** |
| process baseline | 1.73 MiB | 1.73 MiB |
| baseline-adjusted | 77.7 MiB | **74.2 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 5.1× | **4.9× — first sub-5× row** |

The lanes re-ran against main in the same session and the gate is
green across all four workloads — F1 named eval −1.9 % (391.38 ms
median; the 500 ms ceiling stays met), named `save` −5.0 %, criteria,
text and registry eval all improved or flat (registry-eval small
+18.3 % at z 1.6, under the gate's compound threshold). A first branch
suite drifted its criteria window +17 % — the diff-untouched
criteria-open lanes drifted +35–41 % in the same window — and an
interleaved same-binary re-measure read +0.6 %/−0.4 %; the rerun's
gate is clean.

**What remains at the peak — which is staging's splice scan now.**
The workbook's own data (model 20 065 940, computed-so-far
11 603 808, part bytes 7 951 114, shared texts 2 780 178); the
splice-side `scanSheet` scratch (35 188 868 at its top, 76 434 388 of
churn across 59 allocations — each sheet re-scanned so the projection
can splice); the narrowed published pair (4 903 960, already at this
row's floor); and, 300 KB behind at the projection instant, staging's
own records (per-cell 9 600 000, projection targets 5 760 072, the
txn candidate 20 274 118). The drive's publish instant sits at
80 628 739, below the plateau — its records are at their RSS floor,
and cutting them further moves nothing. Cutting further means the
staging era the peak names — the splice-side re-scan and staging's
per-cell records — or renegotiating the ceiling, which at 4.9× is
overdue.

**M10l — the staging row (recorded 2026-08-10).** Same discipline,
and the era trace's first pre-flight outing: every era attributed
before any cut. The M10k baseline reproduced exactly first: RSS
79 577 088 B ×3, peak live 82 759 347, every era and shortlisted site
byte-for-byte.

**The pre-flight read.** The peak was staging's splice-side
`scanSheet` scratch (35 188 868 at its top — the cells/slots/rows
lists at doubled capacity plus the growth transients — 76 434 388 of
churn across 59 allocations); the projection instant sat 300 KB
behind (the scan's retained records 23 922 392 beside the staged set
9 600 000 and the targets 5 760 072); and the drive's publish era sat
at 80 628 739, **2.13 MB under the peak**. The consequence, applied
before cutting: staging-side cuts pay at most 2.13 MB of RSS no
matter how much staging mass they remove. That budget was spent
exactly, and it is why no fourth cut was taken — narrowing the scan's
136-byte cell records or the projection arena's 1.5× chunk would
retire real staging bytes and move zero RSS.
...
| record | before | after | mechanism |
|---|---:|---:|---|
| the splice re-scan's lists | 35 188 868 at the scan instant; 59 allocs / 76 434 388 churn | 23 922 392; one alloc per list | `decode.Options.size_hints`: the model build's scan of the same bytes already produced the counts, recorded per sheet (`WorkbookEnv.sheet_scan_counts`) and replayed at the staging re-scan, so cells (13 601 360), slots (10 001 000) and rows (320 032) allocate once — the scan instant stops being a local maximum at all |
| `Publication` (80 000 staged) | 9 600 000 (120 B) | 6 400 000 (80 B) | `authored` by pointer: the inline `FormulaWrite` was 48 bytes and null on every value-only publication; the pointee outlives the staged set and the projection copies it at the one place it is read |
| `Target` (80 001) | 5 760 072 (72 B) | 2 880 036 (36 B) | `fref` hoisted to a `frefs` side list like `authored`: the inline slice was 16 bytes and absent on every formula without a declared range |

**Result.** Same methodology (ReleaseSafe — the flag is load-bearing —
`/usr/bin/time -l`, usage-invocation baseline, first recalc,
digest-gated fixture):

| | M10k (recorded above) | **M10l** |
|---|---:|---:|
| peak live at the peak instant | 82 759 347 (staging's splice scan) | **80 634 991 (the drive's publish)** |
| `/usr/bin/time -l` peak | 79 577 088 B | **77 496 320 B** |
| process baseline | 1.73 MiB | 1.73 MiB |
| baseline-adjusted | 74.2 MiB | **72.2 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 4.9× | **4.8×** |

The lanes re-ran against main in the same session and the gate is
green across all four workloads on the first pass — no lane flagged,
so no interleaved re-measure was owed: F1 named eval +0.1 %
(398.32 ms median; the 500 ms ceiling stays met), named `save`
−0.4 %, criteria, text and registry eval all improved or flat, and
the largest mover (criteria-open tiny, −59 %) is an improvement.

**What remains at the peak — which is the drive's publish again.**
The workbook's own data (model 20 069 890 — up 3 950 B for the
recorded scan counts — computed-so-far 11 606 082, part bytes
7 951 114, shared texts 2 780 194); the graph's CSR link block
10 210 051 (M10k); the evaluate store's 80 000 × 128 B block
(10 240 000); the iterate execute block 5 759 936 (64 B × the 89 999
admitted edges); the graph build's residual 5 587 480; the published
pair 4 903 960; the journal 1 079 988. Staging's highest instant sits
at 78 087 308, and the txn-prepare era at 79 030 153 — the latter
held up by `replacePart`'s 20 274 118 arena chunk around an 8 365 638
dupe (the 1.5× chunk ladder, M10e's finding, cuttable to an exact
block). M10k's "the drive's records are at their RSS floor" now binds
...
**M10n — the staging-plateau row (recorded 2026-08-10).** Same
discipline. The M10m baseline reproduced exactly first: RSS
74 711 040 B ×3, peak live 78 087 308, every era byte-for-byte.

**The pre-flight read, and the correction it forced.** M10m closed by
naming this row's runner-up gap at 264 KB and calling it the thinnest
the ladder had seen — which, taken at face value, is a reason not to
spend the row. The trace read in full says otherwise. The four eras
that matter:

| era | height | phase | gap under the peak |
|---:|---:|---|---:|
| 21 | **78 087 308** | staging's plateau — the splice scan's records, the staged set, the published pair | — |
| 22 | 77 823 414 | the projection, `resolved.project` | 263 894 |
| 20 | 76 794 991 | the drive's publish | 1 292 317 |
| 23 | 75 140 330 | the plan, `resolved.plan` | 2 946 978 |

Eras 21, 22 and 23 are one phase seen at three instants, and they
hold the same records: the scan's cells and slots are alive across all
three, the staged set across all three, the published pair only at 21.
So a cut aimed at the scan lands on the runner-up *at the same
moment it lands on the peak*, and era 22 cannot inherit the peak by
surviving it. **The era that prices the row is the first one the cut
does not reach — the drive's publish at 76 794 991.** The budget is
therefore 1 292 317 B, not 263 894, and the rule generalizes M10m's:
an era is a runner-up only with respect to a *particular* cut.

The 264 KB reading is not empty, though — it is exactly the price of
the wrong cut. The published pair (3 200 000 + 1 703 960) sits at the
plateau **and** in the drive era, so removing it lowers both together
and stops at era 22, paying 263 894 B for 4.9 MB of work. It was left
alone for that reason.

**The cut** (output bytes unchanged — fixtures and saved archives
byte-identical to the M10m baseline across all four workloads plus f1
small/tiny, 12 comparisons):

| record | before | after | mechanism |
|---|---:|---:|---|
| `CellSlot` (one per `<c>`, 100 010 of them) | 100 B — 10 001 000 | 76 B — **7 600 760** | A `Span` is two `u32`s, so `?Span` pays a tag byte plus three of padding for eight bytes of content, and `CellSpans` carries six: `type_attr`, `v`, `v_content`, `f`, `f_ref`, `is`. `OptSpan` states absence as `end < start` — unreachable for a present span, because every span the scanner records runs from an opening byte to a byte at or after it, which `Span.slice` already asserts — so the six cost eight bytes each and `CellSpans` goes 92 → 68 B. `Target.no_fref` is the same idiom one layer up. The width is now a test (`CellSlot stays at its recorded width`): a field added here costs 100 KB per byte |

**Result.** Same methodology (ReleaseSafe — the flag is load-bearing —
`/usr/bin/time -l`, usage-invocation baseline, first recalc,
digest-gated fixture):

| | M10m (recorded above) | **M10n** |
|---|---:|---:|
| peak live at the peak instant | 78 087 308 (staging's plateau) | **76 794 991 (the drive's publish)** |
| `/usr/bin/time -l` peak | 74 711 040 B | **73 596 928 B** |
| process baseline | 1.72 MiB | 1.72 MiB |
| baseline-adjusted | 69.5 MiB | **68.5 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 4.6× | **4.5×** |

The peak instant did not merely fall — it moved back to the era that
priced the row, at its unchanged height of 76 794 991. The budget was
spent exactly, for the third row running.

The gate is exit 0 in both directions with all 30 lanes ok either way.
The same-binary control was taken first, as M10k's window-drift lesson
requires, and this window was quiet — the identical binary re-run came
back within ±4 % on the sub-2 ms lanes and +0.2 % on F1 named eval,
nothing flagged — so the two brackets against the base (−1.4 % and
+0.6 % on F1 named eval) straddle a neutral change. Named eval
387.6–395.3 ms keeps the 500 ms ceiling met.

**What remains at the peak — which is the drive's publish again.** The
model 20 069 890, the computed-so-far store 11 606 082, the graph's
linked edges 10 210 051, the part bytes 7 951 114, the evaluate store
6 400 000, the iterate `reports` 5 759 936, the graph build residual
5 587 480, the published pair 3 200 000 + 1 703 960, the shared texts
2 780 194. **The row's own finding is on the other side of the ledger:
the cut removed 2 400 240 B and only 1 292 317 of them could be
bought.** Staging's plateau now sits 1 107 923 B *below* the peak with
its retained cells (13 601 360), its slots (7 600 760) and the staged
set (6 400 000) all resident and all worth exactly zero RSS — the
second consecutive row to end with the loser's mass intact, and the
clearest statement yet of what these rows are actually buying: not
memory, but whichever era is currently tallest. The three drive-era
blocks M10m named and protected are now the budget, because the drive
era is now the peak. Below 68.5 MiB is that row — or renegotiating the
ceiling, which at 4.5× and fourteen rows in is overdue.

**M10o — the drive-publish row (recorded 2026-08-10).** Same
discipline. The M10n baseline reproduced exactly first: RSS
73 596 928 B ×3, peak live 76 794 991, every era byte-for-byte.

**The pre-flight read, and the premise it corrected.** This row was
scoped around the four blocks M10m protected and M10n freed, priced
against the plan era: peak 76 794 991, plan era 72 740 090, budget
~4.05 MB. The trace does not support that reading. Between the two
sits an era that holds none of the four:

| era | height | phase | holds any of the four? | gap under the peak |
|---:|---:|---|---|---:|
| 20 | **76 794 991** | the drive's publish | all four | — |
| 21 | 75 687 068 | staging's plateau | **no** | **1 107 923** |
| 22 | 75 423 174 | the projection | no | 1 371 817 |
| 23 | 72 740 090 | the plan | no | 4 054 901 |

Era 21 is the first era a drive-exclusive cut fails to reach, so it
takes the peak the moment the four go, and **the budget for all four
together is 1 107 923 B — not 4 054 901.** Their combined mass is
29 436 199 B. That is M10n's rule applied *before* the cut rather than
discovered after it, which is the only place it pays.

**The block the row was won with is none of the four.** `WorkbookEnv.Cell`
is the run's most-multiplied record — one per coordinate per layer,
held in 64-entry chunks — and the drive publishes a *second* entry at
every formula cell:

| instant | chunks | entries | `Chunk` | chunk bytes |
|---|---:|---:|---:|---:|
| pre-drive | 1 563 | 100 010 | 8 712 B | 13 616 856 |
| post-drive (the peak) | 3 126 | 180 010 | 8 712 B | **27 233 712** |

Every full chunk splits exactly once and fill settles at 57.6/64, so
the directory doubles and the width of one record is paid 180 010
times.

**The cut** (output bytes unchanged — fixtures and saved archives
byte-identical to the M10n baseline across all four workloads ×
tiny/small/named, 24 comparisons):

| record | before | after | mechanism |
|---|---:|---:|---|
| `WorkbookEnv.Cell` (180 010 of them at the peak) | 136 B — 27 233 712 | 88 B — **17 630 640** | Four of ten fields — `cm`, `vm`, `array_ref`, `spill_anchor` — are 52 of the 136 bytes and are zero-or-null on every cell of every §9 workload: cell metadata, a legacy CSE array's declared range, a spill tail's owning anchor. They move behind a `u32` index into a model-arena `extras` list, and `Cell.no_extra` = `maxInt(u32)` states absence — the idiom `Target.no_fref` and M10n's `OptSpan` already use. A cell carrying no `Extra` reads the all-default record, so no caller branches on the index. The width is now a test (`WorkbookEnv.Cell stays at its recorded width`): a byte added here costs 180 KB |

**Result.** Same methodology (ReleaseSafe — the flag is load-bearing —
`/usr/bin/time -l`, usage-invocation baseline, first recalc,
digest-gated fixture):

| | M10n (recorded above) | **M10o** |
|---|---:|---:|
| peak live at the peak instant | 76 794 991 (the drive's publish) | **65 181 445 (the drive's publish)** |
| `/usr/bin/time -l` peak | 73 596 928 B | **63 979 520 B** |
| process baseline | 1.75 MiB | 1.73 MiB |
| baseline-adjusted | 68.5 MiB | **59.3 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 4.5× | **3.9×** |

**The first sub-4× reading the ladder has produced**, and 9 617 408 B
of RSS in one row against 2–3.5 MB for each of its five predecessors.

**The finding — why this row is nine megabytes and its predecessors
were two.** Not a cleverer cut. *A phase-local block's budget is
bounded by the next phase; a record held across every phase is bounded
only by the phases that predate it.* Every row from M10j to M10n cut a
block one phase owned, and each was priced by whichever neighbouring
era survived it. The model's cell record is live continuously from
era 12 to era 23, so the eras that price it are the ones from before
the model exists — era 10, the decode phase, at 52 684 820, **measured
identical before and after the cut**. The budget is 24 110 171 B, the
largest the series has had; 11 613 546 of it was spent, and what
stopped the row was the record's remaining width, not the budget.

Second-order, and M10e's arena ladder paying out for once instead of
costing: chunk *demand* fell 9 603 072 B while the model arena
returned 11 611 854, because a shrinking demand curve crosses a ladder
rung on the way down.

The gate is exit 0 in both directions, all 30 lanes ok either way,
with the same-binary control taken first as M10k's window-drift lesson
requires — and the first attempt at that control earned the lesson
again: it flagged `criteria-eval:criteria_mix_small` at +29.9 % on the
*identical binary*, because a `zig build test` was running beside it.
The window was cleared and the control re-taken, at which point the
same binary reproduced within ±1 % on every substantial lane (F1 named
eval −0.0 %, criteria-eval small −0.1 %). Against that control the
bracket versus the base is −0.7 % on F1 named eval — 378.5 ms, and the
500 ms ceiling stays met. The RSS side was quiet throughout: the M10n
binary re-measured at its recorded 73 596 928 B on all three runs.

**What remains at the peak — the drive's publish, again, with
staging's plateau 1 107 923 B behind it.** The model 12 343 488, the
graph's linked edges 10 210 051, the part bytes 7 951 114, the
publish's own arena chunk 7 720 630, `input.cells` 6 400 000, iterate
`reports` 5 759 936, the graph build residual 5 587 480, the published
pair 3 200 000 + 1 703 960, the shared texts 2 778 502, the drive
journal 1 079 988, the CSR edge data 398 744. The six drive-exclusive
blocks among them carry 29 436 199 B for 1 107 923 B of budget, and
none yields 1.1 MB without moving something this row may not:
`Report.components` is a public contract twenty tests read;
`CellInput.array` is an input-API shape; `Key.Producer`'s two
spellings set the canonical producer order; `Ref` and the graph block
are floored by `Key.Span`; the journal is 27 935 B short even if it
were free. Below 59.3 MiB is a shared cut — the model record again,
where `?ScalarValue` still pays 40 bytes for 32 of content — or the
ceiling conversation, which at 3.9× and fifteen rows in is well past
due.

**M10p — the model-value row (recorded 2026-08-10).** Same discipline.
The M10o baseline reproduced first: peak live **65 181 445** to the
byte, every era byte-for-byte, RSS stable across three runs.

One note on the raw probe, because it looks like drift and is not. The
`/usr/bin/time -l` figure read 63 963 136 B rather than M10o's recorded
63 979 520 — one 16 KiB page low — and the usage-invocation baseline
read 1 802 240 rather than 1 818 624, low by the *same* page. The
baseline-adjusted number, which is what this section records, therefore
reproduced **exactly**: 62 160 896 B. Both probes were stable across
five samples. A page of argv and environment moves the raw figure and
cancels out of the adjusted one, which is why the adjustment is in the
methodology at all.

**The pre-flight read.** M10o closed by naming this cut, so the trace
was read to price it rather than to find it:

| era | height | phase | holds the model's cell record? |
|---:|---:|---|---|
| 20 | **65 181 445** | the drive's publish | yes |
| 21 | 64 073 522 | staging's plateau | yes |
| 22 | 63 809 628 | the projection | yes |
| 15 | 60 448 641 | the graph build | yes (pre-drive half) |
| 10 | 52 684 820 | the decode phase | **no** |

A drive-exclusive cut is capped at era 21's 1 107 923 B. This record is
held at 20/21/22/23 together, so a cut to it lowers all four and the
first era it fails to reach sets the price — **era 15, budget
4 732 804 B** — with era 10 the true floor once the pre-drive half is
counted. The planned 1 600 512 B sits well inside either reading, so
the budget was never the binding constraint. What happened instead is
that it did not bind in a way the trace could express at all.

**The cut** (output bytes unchanged — fixtures and saved archives
byte-identical to the M10o baseline across all four workloads ×
tiny/small/named, 24 comparisons):

| record | before | after | mechanism |
|---|---:|---:|---|
| `WorkbookEnv.Cell` (180 010 at the peak) | 88 B — 17 630 640 of chunk | 80 B — **16 030 128** | `v` was `?ScalarValue`: 40 bytes to carry 32. `ScalarValue`'s payload is eight-aligned, so the optional's presence flag cannot share the union's tag word and opens a new one. `OptValue` makes absence the sixth case of that same tag — the idiom `Target.no_fref` and M10n's `OptSpan` already use. `absent` stays distinct from `blank`: `get()` yields the `?ScalarValue`, `orBlank()` performs the merged view's collapse at the read site (§5.6f) rather than in storage, and a test pins that distinction beside the width |

**Result.** Same methodology (ReleaseSafe, `/usr/bin/time -l`,
usage-invocation baseline, first recalc, digest-gated fixture):

| | M10o (recorded above) | **M10p** |
|---|---:|---:|
| peak live at the peak instant | 65 181 445 (the drive's publish) | **65 181 995 (the drive's publish)** |
| `/usr/bin/time -l` peak | 63 963 136 B (re-measured) | **62 390 272 B** |
| process baseline | 1 802 240 B | 1 818 624 B |
| **baseline-adjusted** | **62 160 896 B — 59.28 MiB** | **60 571 648 B — 57.77 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 3.9× | **3.8×** |

**The finding — peak live did not move, and RSS fell 1 572 864 B
anyway.** Peak live went *up* 550 bytes. The era deltas are uniform and
tiny:
...
In the era trace's own currency this row bought nothing at the peak.
The RSS probe, measured three times at 62 390 272 B, disagrees by
1.57 MB. Both are right, and the gap between them is the row's lesson.

`Cell` chunks are held in the model arena, and the profiler sits at the
allocator boundary — so what it records for an arena is the arena's
*backing chunk requests*, not the bytes written into them. Removing
1 600 512 B of demand did not move the ×1.5 ladder off its rungs: the
model arena's request fell 4 884 B (12 343 488 → 12 338 604), the
publish's chunk 514 B (7 720 630 → 7 720 116), and a neighbouring
arena's series reshuffled +5 948 — net +550. The saving came out of the
*fill* inside chunks already mapped, and **a mapped page that is never
written is never resident**. Predicted fill drop
3126 × (5640 − 5128) = 1 600 512; measured RSS drop 1 572 864, the
remainder page rounding.

**The heap profiler prices the ladder; RSS prices the fill.** M10o's
second-order note was this same mechanism with the sign reversed —
there the shrinking demand curve *crossed* a rung, so the arena
returned 11 611 854 B for a 9 603 072 B cut and peak live over-reported
the win. Here it crossed nothing and under-reported it completely. The
consequence for the method: **the era trace's budget is denominated in
peak-live bytes, so for an arena-held record it bounds what the trace
will show, not what RSS will give.** This row spent zero of its
4 732 804 B budget by that reckoning and still collected 1.57 MB. A
budget of zero is not a reason to skip a cut when the block is
arena-held — the fill has to be priced separately, and the instrument
that reads it is the RSS probe, not the trace.

The gate is exit 0 in both directions, all 30 lanes ok either way, with
the same-binary control taken first as M10k's and M10o's window-drift
lesson requires. The control was clean this time — F1 named eval
+1.2 %, criteria-eval small −0.9 %, text-eval small +0.7 %, every
substantial lane inside ±2.2 % — so the bracket is trustworthy: F1
named eval **386.72 → 375.09 ms, −3.0 % at z −3.2**, and the 500 ms
ceiling stays met; named `save` −0.4 %; criteria, text and registry
eval flat-to-faster. (The sub-2 ms `open` lanes swing ±18 % in the
control as well as the bracket; they sit under the gate's 0.5 ms
absolute floor and carry no signal.)

**What remains at the peak — the drive's publish, unmoved.** The peak
instant, the runner-up order and every block in them are what M10o
left: the model 12 338 604, the graph's linked edges 10 210 051, the
part bytes 7 951 114, the publish's own arena chunk 7 720 116,
`input.cells` 6 400 000, iterate `reports` 5 759 936, the graph build
residual 5 587 480, the published pair 3 200 000 + 1 703 960, the
shared texts 2 784 450, the drive journal 1 079 988, the CSR edge data
398 744. The six drive-exclusive blocks are still floored by the
contracts M10o listed and this row did not renegotiate. What this row
adds is that peak live is now a *poor* guide to what is left: several
of those blocks are arena-held, and their fill has never been measured.
At 3.8× and sixteen rows in, the ceiling conversation is the next row,
not another cut.

**M10r — the in-place-publication row (recorded 2026-08-11).** Same
discipline. The baseline reproduced first, exactly: peak live
**65 181 995**, eras 10 = 52 684 820 / 15 = 60 441 827 / 20 = 65 181 995
/ 21 = 64 074 072 byte-for-byte, RSS 62 439 424 B across three runs, and
the fill probe's model arena at capacity_end 22 842 794 / fill_peak
18 157 309 / unfilled 4 685 485.

One bookkeeping note first. The baseline above is **M10q's**, not
M10p's: M10q compiled the fill probe in, and §3's own measurement put
its cost at 32 768 B raw. The usage-invocation baseline rose by one
16 KiB page with it, so the adjusted figure moved by the other page —
60 571 648 → **60 588 032 B**, 57.77 → 57.78 MiB. That row recorded the
cost and did not restate this section; it is restated here so the
before-figure below is the one actually measured on the binary this row
cut.

**The pre-flight read.** goal_codex.md §2 priced this cut before it was
written, and the price was the whole reason to write it that way:

| era | height | phase | reached by a publication-side cut? |
|---:|---:|---|---|
| 20 | **65 181 995** | the drive's publish | yes — the peak |
| 21 | 64 074 072 | staging's plateau | yes |
| 15 | 60 441 827 | the graph build | **no — it predates publication** |
| 10 | 52 684 820 | the decode phase | no |

Era 15 is the budget: **4 740 168 B**, and the row was accepted on the
understanding that it could not buy more peak live than that no matter
how much record demand it removed.

**The cut.** The drive published a *second* `Cell` at every formula
cell — `Cell`'s own header has said so since M10o. The two records were
never both read: the computed one shadowed the stored one for every
merged read, and the stored one answered `formulaAt`, `dialectOf` and
`svOccupied` out of `formula_text`, `cache` and `extra`, which a
computed record does not carry. So the publication overwrites `v` on the
record already there and leaves the other four fields alone.

That cannot be staged, and the reason is the whole design: the engine's
journal holds only a `CellRef`, so a refusal can uncover a lower layer
but cannot restore an overwritten one. `iterate.Options.retraction`
gains a second policy — `.candidate_discard`, which keeps no journal and
never retracts — and `recalc_run` is its one caller, because its
`WorkbookEnv` is per-run and dies at `defer model.deinit()` on every
path while the live workbook is touched only at `Candidate.swap`.
`journal` stays the default and the generic `Host` contract is unchanged;
`retractResult` asserts the pairing rather than documenting it.

Three exclusions, each with a test that fails without it: **never a
spill tail** (its `v` belongs to the anchor that placed it, and the
C1-over-B1's-tail fixture requires retraction to reveal it again), **a
formula record only** (a stored value has no relationship to the run),
and **never a published `.blank`** — `dupeValue` folds blank to
`.absent`, and `.absent` on a record carrying `formula_text` is this
type's spelling of *uncached*, so writing it in place would make
`isUncached` answer the opposite of the truth.

**Result.** Same methodology (ReleaseSafe, `/usr/bin/time -l`,
usage-invocation baseline, first recalc, digest-gated fixture):

| | M10q (restated above) | **M10r** |
|---|---:|---:|
| peak live at the peak instant | 65 181 995 (the drive's publish) | **60 441 827 (the graph build)** |
| `/usr/bin/time -l` peak ×3 | 62 439 424 B | **56 672 256 B** |
| process baseline | 1 851 392 B | 1 851 392 B |
| **baseline-adjusted** | **60 588 032 B — 57.78 MiB** | **54 820 864 B — 52.28 MiB** |
| vs the 3 × model-bytes ceiling (15.15 MiB) | 3.8× | **3.5×** |

The era trace lands where the pre-flight put it, which is the first time
in this ladder that has happened without a correction:

| era | before | after | Δ |
|---:|---:|---:|---:|
| 10 | 52 684 820 | 52 684 820 | **0** |
| 15 | 60 441 827 | 60 441 827 | **0 — and now the peak** |
| 20 | 65 181 995 | 56 375 785 | −8 806 210 |
| 21 | 64 074 072 | 56 346 078 | −7 727 994 |
| 22 | 63 810 178 | 56 082 184 | −7 727 994 |
| 23 | 61 127 094 | 53 399 100 | −7 727 994 |

Era 20 loses exactly the three allocations the prediction named — `id82`
7 720 116 (the publication's own arena chunk), `id68` 1 079 988 (the
journal's reserve) and `id21`'s publication-triggered step of 7 878,
8 807 982 against 8 806 210 measured, 1 772 of residual churn elsewhere.
Peak live falls **4 740 168 B: the era-15 budget, spent to the byte.**

**The finding — a cut can pay MORE than its peak-live budget.** RSS fell
**5 767 168 B**, which is **1 027 000 past the cap**. The cap was not
wrong; it bounds the *trace*. RSS sat 2 742 571 B below peak live before
the cut and 3 769 571 B below it after, and that widening is the extra:
the removed bytes were touched fill inside chunks whose ×1.5 ladder did
not step, so their pages left residency while the era heights that hold
them did not move. The fill probe reads it directly — the model arena's
`fill_peak` 18 157 309 → **10 122 389** (−8 034 920, against 8 015 064
predicted from 1 563 × 5 128, the extra 19 856 being the sheet
directory's own growth), while `capacity_end` fell only 7 727 970 and
**unfilled capacity rose**, 4 685 485 → 4 992 435:

| arena | capacity_end | fill_peak | unfilled |
|---|---:|---:|---:|
| model | 22 842 794 → **15 114 824** | 18 157 309 → **10 122 389** | 4 685 485 → 4 992 435 |
| stage | 8 365 688 (unmoved) | 7 496 961 (unmoved) | 868 727 (unmoved) |
| **totals** | 31 214 100 → **23 486 130** | 25 657 254 → **17 622 334** | 5 556 846 → 5 863 796 |

M10p was this mechanism with the budget spent at **zero** and 1.57 MB
collected; this row is the same mechanism with the budget spent
**exactly** and 1.03 MB collected past it. Between them the method
question §3 was built to settle is settled: **the peak-live budget
bounds what the era trace will show, and it bounds RSS in neither
direction.** A row is priced by the trace and paid by the probe, and
both numbers have to be recorded.

**The gate, and the one figure this row does not have.** `--gate` is
exit 0 in both directions, all 30 lanes ok either way, with the
same-binary control taken first — the tightest control this ladder has
recorded, every substantial lane inside ±0.4 %. Against it the cut is
faster everywhere that matters: F1 named eval 535.17 → **517.47 ms**
(−3.3 %), named `save` −2.6 %, criteria/registry/text eval −0.8 % to
−3.8 %. Fewer records to insert, no chunk splits at the publish and no
journal appends is a wall-clock saving as well as a memory one.

**But the 500 ms ceiling was not measurable in this window, and the row
does not claim it.** 517 ms is over the line, and the reason is the
host. M10p's own commit was rebuilt and measured beside this one — four
binaries interleaved in a single window, three rounds, order reversed in
round 2:

| binary | evaluate | rounds |
|---|---:|---|
| M10p `9127713` | **542.65 ms** | 542.1 / 544.5 / 542.7 |
| review fixes `611f5c7` | 541.34 ms | 541.3 / 540.1 / 541.5 |
| main `d8e16f1` (M10q) | 543.38 ms | 544.6 / 543.4 / 542.3 |
| **M10r** | **524.14 ms** | 524.3 / 522.7 / 524.1 |

M10p's binary records **542.65 ms** where M10p's row recorded 375.09.
Three commits sit inside 2.0 ms of each other, so nothing between M10p
and here regressed the lane; the host is running ~1.45× the session that
set the 375–433 ms band. A fifteen-minute idle cooldown moved it to
540.40 (cut 522.86), so it is not thermal soak either. Anchored to
M10p's binary in the same window the cut is **540.40 → 522.86 ms,
−3.2 %**, which places its M10p-equivalent at **≈362.9 ms** — better
than M10p's own recorded figure, and **recorded as derived rather than
as a measurement.** The absolute ceiling owes a clean-host re-measure;
what this window can prove is the direction, and the direction is down.

(Two measurement notes, because both nearly became records. A first
bench pass was discarded outright: `mediaanalysisd` was at 193 % CPU
through it and it read named evaluate at 542 ms *on main* — a fabricated
regression, caught only because the same-binary control is taken first.
And `pmset` reporting `powermode 1` on AC is the explicit high-power
setting on this part, not Low Power Mode — the battery section reads 0 —
so it is not an explanation for the 1.45×.)

**What remains at the peak — the graph build, for the first time since
M10j.** Its at-peak table is byte-identical to the baseline's, which is
the direct evidence that the cut did not reach it: the model 12 330 726,
the build's own scratch 11 614 712, the link block 10 210 051, the part
bytes 7 951 114, the owner map 7 680 000, the bridge's input records
6 400 000, the shared texts 2 784 450, the walk logs 960 000, 480 128 of
scan records, and 7 436 below the tabulated twelve. Nothing on the
result side reaches any of it, which is exactly what the budget said,
and the only scheduled row that does is goal_codex.md §6's columnar
...
### 9.1g M10x — the list that was the maximum, and the ladder closed (2026-08-12)

§9.1f spent §9.1c's budget, moved the resident maximum into the decode
half, and left **1 343 488 B** between it and the runner-up. It also left
a specific claim behind: that §9.1c's candidate 2 — the first scan's
1 431 436 B of unhinted list slack — had gone from worth nothing to worth
up to that gap, because "nothing about the waste changed; this row
...
**229 376 B**, which is the ladder closing.

#### Candidate 2, priced directly: zero, and not marginally

§9.1c priced it by argument (it lives in era 10, era 10 is below the
maximum). §9.1f re-priced it by argument (era 10 *is* the maximum now).
Both arguments are about **which era** the waste lives in, and neither
measured it. So this row builds the knob §9.1f asked future rows to reach
for — `--scan-padding N`, N bytes of never-written slack held for exactly
the lifetime of one `scanSheet`, on the same binary as the unpadded arm:

| padding across the scan | baseline-adjusted | Δ |
|---:|---:|---:|
| 0 | 51 150 848 | — |
| **1 431 436** — candidate 2's own figure | 51 150 848 | **+0** |
| 4 194 304 | 51 150 848 | **+0** |
| 11 534 336 | 54 755 328 | +3 604 480 |
| 16 777 216 | 59 998 208 | +8 847 360 |
...
the scan's span is 7 929 856 B below the maximum, and lowering an instant
...
not a span.** The maximum was in the decode *half*, which §9.1f said
correctly; the scan is one span inside that half, and the maximum is in
the next one. A candidate has to be live at the instant, not merely in
the neighbourhood.

#### What the maximum actually is

One `gpa` list, growing out of place. `classifySheet` allocates one
`Entry` per cell carrying an `<f>` and discovers how many there are by
appending; at the last growth the old buffer and the new one are both
...
|---|---:|
| `@sizeOf(Entry)` | 128 |
| entries the sheet needs — 80 000 | **10 240 000** |
| entries capacity at hand-off — 106 705 | **13 658 240** |
| … of which slack | 3 418 240 |
| the abandoned buffer at the last growth — 71 135 | **9 105 280** |
| **live at the maximum** | **22 763 520** |
| scan cell records, beside it | 13 601 360 |
| scan slot records | 7 600 760 |
| scan row records | 320 032 |
| the sheet part every span borrows | 5 288 922 |
| `out`, one `SharedText` per cell — allocated *after* | 4 800 480 |
| groups, masters | 0, 0 |
...
and the copy holds both rungs at once. Neither figure is a page count:
they are logical bytes, and what converts them is the knob below.

#### The knob, one span later: 1.000 B of page per byte

`--classify-padding N` is the same probe as `--scan-padding`, moved one
span along, into `classifySheet`:

| padding across classify | baseline-adjusted | Δ |
|---:|---:|---:|
| 0 | 51 150 848 | — |
| 1 048 576 | 52 199 424 | **+1 048 576** |
| 2 097 152 | 53 248 000 | **+2 097 152** |
| 4 194 304 | 55 345 152 | **+4 194 304** |

**1.000 B of resident page per byte, three times, exact, every reading
byte-identical across three repetitions.** The same knob at two instants
of the same half: 0.000 in the scan, 1.000 in the classify. That pair is
the whole pricing argument, and it is one binary, so binary layout
cancels exactly rather than approximately.

The budget is the other end of the same measurement. §9.1f's own knob,
turned here: 2 097 152 B of never-carved slack on the graph block reads
**+802 816**, absorbing **1 294 336 B** — and the `pages` instrument
reads the maximum at 51 232 768 and the runner-up at 49 938 432, a gap of
**1 294 336 B**. Two instruments with different baselines, one figure.
(§9.1f recorded 1 343 488 on its binary; this pair reads three pages
lower, which is the ±1-page layout sensitivity that section documented,
here worth 3.8 % of the gap.)

#### The candidates, each priced before it was written

| candidate | bytes at the maximum | verdict |
|---|---:|---|
| the entries list's abandoned buffer | **9 105 280** | shipped |
| the entries list's capacity slack | **3 418 240** | shipped |
| §9.1c's candidate 2 — the scan's list slack | 1 431 436 | **zero** — 7 929 856 B below the envelope, measured |
| `out`, sized to cells rather than formulas | 960 480 | **zero** — allocated after `classifySheet` returns |
| the scan's slot records, whose length is all the model build reads | 7 600 760 | **zero at this budget** — live at the maximum, but the budget is spent twice over without it |
| the scan's own growth copies | — | **zero** — same span as candidate 2 |

Removable at the maximum: **12 523 520 B**, which is 9.7× the 1 294 336 B
the envelope allows. So the prediction stated before the cut was that it
would collect the gap and the maximum would move to the graph half.

#### The cut

One statement, plus the count that makes it possible.

1. **`decode.Sheet` reports `formula_cells`** — how many of the cells it
   returns carry an `<f>`. The walk counts as it appends, so it costs
   nothing: this is M10l's `size_hints` one boundary earlier, and the
   same reasoning (the scan already knows; the next pass was rediscovering
   it by walking a ladder).
2. **`classifySheet` takes it as a hint and allocates once.** The hint is
   clamped to `cells.len` — a hint is a caller's arithmetic and an
   unclamped one reaching an allocation size is the failure mode §9.1f's
   review found in the block knob. A wrong hint costs only the growth
...
| arm | baseline | raw peak | baseline-adjusted |
|---|---:|---:|---:|
| §9.1f tip (`aaddf01`), rebuilt today | 1 835 008 | 52 969 472 | 51 134 464 |
| + census + knobs, cut reverted | 1 818 624 | 52 969 472 | 51 150 848 |
| **+ the cut** | 1 818 624 | 51 511 296 | **49 692 672** |
...
recorded 51 134 464 exactly, so no session-drift correction applies to
this row's figures.

The instruments themselves cost **one page** (51 134 464 → 51 150 848),
and the cut is measured against the arm that carries them.
...
|---|---|---|
| eras detected | 22 | 18 |
| **maximum** | era 10, fill 8 352 — **51 232 768** | era 14, fill 8 533 121 — **49 774 592** |
| runner-up | era 18, fill 8 533 121 — 49 938 432 | era 15, fill 8 533 121 — 49 545 216 |
| **gap** | **1 294 336** | **229 376** |
| decode-half eras | 11 | 7 |
| **highest decode-half era** | **51 232 768** | **39 387 136** |

`sampled_peak == getrusage_maxrss` to the byte on every run, coverage
1.0000. The maximum moves out of the decode half — identified by its
measured arena fill, never by a phase name the instrument does not report
— and the decode half lands **10 387 456 B** below the envelope. The
`pages` instrument's own delta, 51 232 768 − 49 774 592 = **1 458 176**,
is the gate delta to the byte, from an instrument with a different
baseline.
...
| arm | baseline | raw peak | baseline-adjusted |
|---|---:|---:|---:|
| cut reverted, ReleaseFast | 1 736 704 | 51 314 688 | 49 577 984 |
...
maximum sits at fill 8 533 121 — the graph half — on *both* arms, while
...
- **All of it is a lane effect, and that is measured, not attributed.**
  ReleaseFast moves +32 768 the wrong way, and its own maximum never was
  in this half.
...
| Doc | Claim | Action |
|---|---|---|
| `docs/plans/archive/post-0.2.9-roadmap.md:144,203,208-211,487-488,596-607` | D1 deferred / exclusions | **M-1** flip |
...
| `docs/plans/archive/editor-rebase.md:359` · `docs/plans/archive/workbook-overlay.md:287` · `docs/plans/archive/writer-rebase.md:575` · `docs/plans/archive/structural-edits.md:121` ("Excel will recompute") | no-evaluation statements | **M-1** historical labels |
...
| Row | Disposition |
|---|---|
| `docs/plans/archive/post-0.2.9-roadmap.md` | ✅ flipped — D1 table row, TL;DR, sequencing graph, iter-C1 note, and the whole "D1. Formula evaluator — minimal (deferred indefinitely)" section, which now points here and keeps its pre-reversal critique as a labelled quote |
...
| **D · out-of-scope, non-formula** | `pkg/store.zig:13` · `pkg/typed_parts/styles_xml.zig:25` · `pkg/drawings.zig:4` · `pkg/drawing_emit.zig:58` · `src/writer.zig:30-34` · `src/xlsx.zig:1819` · `pkg/workbook.zig:929` · `docs/package-layer.md:125` · `docs/plans/archive/load-modify-save.md:73,178,258` · `docs/plans/archive/cell-mutate.md:261` · `docs/plans/emb-4-compat-matrix.md:176` · `docs/plans/refusal-audit.md:189` · `docs/plans/embeddings-in-xlsx.md:8,638,847,892` · `docs/plans/archive/workbook-overlay.md:216,280` · `docs/plans/archive/editor-rebase.md:352` · `docs/plans/archive/writer-rebase.md:572` · `docs/plans/archive/structural-edits.md:276` · `docs/vs_calamine.md:91,145` · `docs/plans/archive/post-0.2.9-roadmap.md:616` · `README.md:481` · `bindings/python/README.md:257` · `goal_plan.md:37` · `bindings/python/zlsx/__init__.py:2475` | Scope statements about ZIP64, styles, charts, pivots, `.xls`/`.xlsb`, Google Sheets, encryption, SST compaction — none about evaluation |
| **E · `<dimension>` / Excel-recomputes-on-open** | `pkg/sheet_edit.zig:375,650,674,1146,1169,1223` · `docs/plans/archive/load-modify-save.md:27,158,279` · `docs/plans/archive/cell-mutate.md:6,49,67,207,255` · `docs/plans/archive/structural-edits.md:100` · `docs/plans/archive/post-0.2.9-roadmap.md:148` · `docs/plans/refusal-audit.md:62` | Structural-edit behaviour: zlsx leaves `<dimension>`/totals for Excel to fix on open. Not a claim about computing formulas |
