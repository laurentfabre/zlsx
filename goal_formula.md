...
Tier **D1**. One PR per row — **50 rows, M-1 … M10i** (count = the table; every v1 function name frozen — no ellipses);
counts regenerate from the frozen registry inventory (M3a). v1 = M9d; M10a–M10i are the post-v1 rows.
