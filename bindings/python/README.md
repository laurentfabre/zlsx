...
| `ZipBombSuspected` | any editor read that materialises parts (`pivots()`, `defined_names()`, `conditional_formats()`, the structural edits) on an archive past the decompression caps — the S1 `decompress_limits` verdict, typed rather than a plain `ZlsxError` |
