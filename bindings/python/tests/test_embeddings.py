...
def test_cell_carrier_survives_a_numbers_shaped_strip(tmp_path: Path):
    """The opt-in carrier, from Python.

    `zlsx-emb4-fixture --cells` enables the cell carrier — the only one
    Numbers preserves. After removing everything Numbers removes, the
    record must still come back, and report `cell_data` so the caller
    can tell which carrier answered.
    """
    import re

    src = tmp_path / "cells.xlsx"
    subprocess.run([str(EMB4_GEN), str(src), "--cells"], check=True, capture_output=True)

    out = tmp_path / "cells-numbers.xlsx"
    zin = zipfile.ZipFile(src)
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zo:
        for name in zin.namelist():
            if name.startswith("xl/zlsxEmbeddings/") or name.startswith("customXml/"):
                continue
            ...
            if name == "xl/workbook.xml":
                data = re.sub(
                    r"<definedName[^>]*>.*?</definedName>", "", data.decode(), flags=re.S
                ).encode()
            elif name == "xl/_rels/workbook.xml.rels":
                data = re.sub(
                    r"<Relationship[^>]*zlsxEmbeddings[^>]*/>", "", data.decode()
    ...
    with zlsx.embeddings(out) as e:
        assert e.stripped, "cell carrier should have survived"
        assert e.carrier == "cell_data"
        assert e.model == "emb-4-fixture-v1"
        assert [c.id for c in e.coverages] == ["title", "body"]
