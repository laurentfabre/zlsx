...
| S7b | **Asked 2026-08-28** — the full analysis is `docs/plans/s7b-cache-policy.md`; its §7 is the question set. (Q1) **The cache policy**: A — move `worksheetSource@ref` (and each `rangeSet@ref`) by range semantics and leave the records / inventories as saved, the posture Excel itself takes on the same edit; or B — also rewrite `pivotCacheRecords`, every `sharedItems` inventory and every `<x v>` index in every table on the cache, which assumes the snapshot is current (unverifiable) and is S8's cache builder in disguise. **Recommended A.** (Q2) The refresh marker: A0 never; A1 `refreshOnLoad="1"` on a cache whose rectangle the edit *changed* (insert / delete inside; a pure shift stays byte-faithful) — Excel then refreshes on open and shows the *"Refresh data when opening the file"* option checked (best-effort: inert under `enableRefresh="0"` and programmatic opens; refreshes every consumer of a shared cache); or A2 `invalid="1"` on the same condition — the spec's "needs refreshing" state flag, no user option flipped, Excel's response at open oracle-pending. **Recommended A1, or A2 if the oracle shows Excel refreshes on `invalid` and clears it** (zlsx is headless; nobody clicks Refresh). (Q3) Extend the marker to `setCell` writes inside a resolved source rectangle as a save-time pass in S7b's second PR, so the row leaves one rule. **Recommended yes.** (Q4) Unresolved sources on a row edit of a sheet that hosts nothing, split: a dangling spelling (nothing local affected) vs a dynamic name (`OFFSET(Report!…)` — reads a local sheet the resolver cannot bound; the name sweep already moves its body). Admit both untouched, admit + mark the dynamic case when the edited sheet appears in its body unless the dry-run body spells `#REF!` (recommended), or refuse workbook-wide. (Q5) An unknown cache source `type` on a non-host sheet: admit (recommended — the definition names no sheet) or refuse workbook-wide as S7a does for hosts. |
