...
_Created: 2026-08-26 · `main` at `d99a386` · v0.8.0, Zig 0.16.0 · Status: **S0 done (gate answered 2026-08-26) · S1 / S2 next**_
...
| S1 | Core reader decompression defenses | planned | 1 wk | — | `src/xlsx.zig::extractEntryToBuffer` gets the package path's per-part cap + compression-ratio checks plus a **new** aggregate budget (S0 found the package path has none either — `pkg/store.zig` checks per-part and ratio only, and only when decompressing). That also closes the editor hole: `Editor.open` runs its structural scan (Zip64 / split / encrypted / data-descriptor refusals, no size checks) and then `Book.open`, which decompresses every sheet uncapped before the editor's own capped `readEntry` ever runs. Hostile-archive tests; typed CLI exit mapping. A safety gap, not polish. |
...
| S6 | Pivot topology + typed read | planned | 3–4 wk | S1 | Corpus-driven inventory of the full graph (`<pivotCaches>` → cacheDefinition → `worksheetSource` → pivotTable → cacheRecords, Strict + Transitional URIs); audit that #139's guard truly sees every source sheet; conservative typed **read-only** model, raw parts stay byte-preserved; 4-surface read (`zlsx pivots`). |
| S7a | Pivot edit lift: output-location | planned | 1–2 wk | S6, S2 | Row/col edits on sheets that only *host* a pivot rewrite `pivotTableDefinition/location@ref`; everything else still refuses. |
...
| S1 | Confirm the three limits (per-part cap, ratio, aggregate budget) and their values — reuse the package path's 512 MiB numbers or set new ones. |
...
| S6 | Approve the typed-read surface (what `zlsx pivots` NDJSON exposes) before it freezes. |
