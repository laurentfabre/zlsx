...
| S7b | **Asked 2026-08-28** — the full analysis is `docs/plans/s7b-cache-policy.md`; its §7 is the question set. (Q1) **The cache policy**: A — move `worksheetSource@ref` (and each `rangeSet@ref`) by range semantics and leave the records / inventories as saved, the posture Excel itself takes on the same edit; or B — also rewrite `pivotCacheRecords`, every `sharedItems` inventory and every `<x v>` index in every table on the cache, which assumes the snapshot is current (unverifiable) and is S8's cache builder in disguise. **Recommended A.** (Q2) The refresh marker: A0 never, or A1 `refreshOnLoad="1"` on a cache whose rectangle the edit *changed* (insert / delete inside; a pure shift stays byte-faithful) — Excel then refreshes on open and shows the *"Refresh data when opening the file"* option checked. **Recommended A1** (zlsx is headless; nobody clicks Refresh). (Q3) Extend the marker to `setCell` writes inside a resolved source rectangle as a save-time pass in S7b's second PR, so the row leaves one rule. **Recommended yes.** (Q4) Unresolved / unknown sources on a row edit of a sheet that hosts nothing: admit (recommended — a stale source reference is recoverable; Excel never refuses) or refuse workbook-wide. |
