...
/// removal), the package's own `MissingContentTypes`, and the
/// delete's verdicts should the cells sheet be present
/// (`MissingRelationship`, a carrier the cross-sheet sweeps cannot
/// read — `MalformedChartXml`, `MalformedExtensionXml`, …). A -2 or
/// -3 after the first removal (an allocation failure, a content-types
/// part a removal cannot patch) leaves the staged set partially
